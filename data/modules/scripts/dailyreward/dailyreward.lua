...
DailyRewardSystem = {
    Developer = "Marcos G (westwol)",
    Version = "1.0",
    lastUpdate = "24/09/2018 - 23:16"
}
...
-- Core functions
  ...
  DailyReward.loadDailyReward = function(playerId, source)
    ...
    if player:getClient().os == CLIENTOS_NEW_WINDOWS then
      ...
      player:sendDailyRewardOldClient()
    end
    return true
  end
...
function Player.sendDailyRewardOldClient(self)

    local playerId = self:getId()

    local window = ModalWindow {
      title = 'Reward Wall',
      message = '---------------| Welcome to your daily reward wall |---------------\nStreak level: ' .. self:getStreakLevel() .. ' \nReward: #' .. self:getDayStreak() + 1 ..'\nCollection Tokens: ' .. self:getCollectionTokens() ..'\n\nYou can open this dialog anytime using command: !dailyreward'
    }

    -- choices would be items if days
    local dailyTable = DailyReward.rewards[self:getDayStreak() + 1]
    if not dailyTable then
      self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Something went wrong and we can not process this request.")
      return false
    end
    ...
    local pickReward, history, bonuses

    bonuses = window:addChoice('Resting Area Bonuses (' .. self:getDailyRewardBonusesCount() ..')')
    local rewardmsg = "available"
    if DailyReward.isRewardTaken(playerId) then
      rewardmsg = "picked"
    end
    pickReward = window:addChoice('Daily Reward ['.. rewardmsg ..']')
    history = window:addChoice('History')

    window:addButton('Ok', function(button, choice)
    if choice == bonuses then
      local infowindow = ModalWindow {
        title = 'Resting Area Bonuses',
        message = 'These are your resting area bonuses\n\nHurry up! Pick up your daily reward withing the next '.. string.diff(Game.getLastServerSave() + DailyReward.serverTimeThreshold - os.time()) ..' (before the next regular server save) to raise your reward streak by one and be benefited from the next bonuses:'
      }

      for i = 2, #DailyReward.strikeBonuses do
        infowindow:addChoice(i .. ' - ' ..DailyReward.strikeBonuses[i].text .. ' ['.. self:isBonusActiveById(i) ..']')
      end

      local bonusDesc = {
        [1] = "2nd level streak bonus [".. self:isBonusActiveById(2) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 2.\n\nThis bonus grants you the ability of hitpoints regeneration inside resting areas.",
        [2] = "3rd level streak bonus [".. self:isBonusActiveById(3) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 3.\n\nThis bonus grants you the ability of mana regeneration inside resting areas.",
        [3] = "4th level streak bonus [".. self:isBonusActiveById(4) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 4.\n\nThis bonus grants you the ability of stamina regeneration inside resting areas.\n\nIf your stamina is below 40 hours, you stamina will be recovered by 1 every 3 minutes else you will be recoverd by 1 every 10 minutes.",
        [4] = "5th level streak bonus [".. self:isBonusActiveById(5) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 5.\n\nThis bonus grants you the ability of double hitpoints regeneration inside resting areas.",
        [5] = "6th level streak bonus [".. self:isBonusActiveById(6) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 6.\n\nThis bonus grants you the ability of double mana regeneration inside resting areas.",
        [6] = "7th level streak bonus [".. self:isBonusActiveById(7) .."]\n\nThis bonus is activated if you have reached a streak level of atleast 7.\n\nThis bonus grants you the ability of soul points regeneration inside resting areas.",
      }

      infowindow:addButton('Details', function(button, choice)
        for i = 1, #bonusDesc do
          if choice.id == i then
            local bonusInfo = ModalWindow {
              title = 'Resting Area Bonuses (details)',
              message = bonusDesc[i]
            }
            bonusInfo:addButton('Back', function(button, choice) infowindow:sendToPlayer(self) end)
            bonusInfo:sendToPlayer(self)
          end
        end
      end)

      infowindow:addButton('Back', function(button, choice) self:sendDailyRewardOldClient() end)
      infowindow:sendToPlayer(self)
    elseif choice == pickReward then
      if DailyReward.isRewardTaken(playerId) and not DailyReward.testMode then
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have already collected your daily reward.")
        return false
      end
      -- Potions, runes, etc
      if (dailyTable.type == DAILY_REWARD_TYPE_RUNE_POTION) then
        sendItemChoose(self, {}, 1)
      end
      -- temporary item, to be worked
      if(dailyTable.type == DAILY_REWARD_TYPE_STORAGE) then
        local desc = ""
        for i = 1, #reward.things do
          for j = 1, #reward.things[i].storages do
          ...
          if i ~= #reward.things then
            desc = desc .. reward.things[i].name .. ", "
          else
            desc = desc .. reward.things[i].name .. "."
          end
        end
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You have claimed: %s on %d day streak.", desc, self:getDayStreak()))
        DailyReward.insertHistory(self:getGuid(), self:getDayStreak(), "Claimed reward no. " .. self:getDayStreak() + 1 .. ". Picked reward: " .. desc)
        DailyReward.pickedReward(playerId)
        self:sendDailyRewardOldClient()
        return false
      end
      -- xp bonus
      if (dailyTable.type == DAILY_REWARD_TYPE_XP_BOOST) then
        ...
        window:addButton('Ok', function(button, choice)
          self:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You have claimed Bonus XP %d minutes on %d day streak.", reward.xpminutes * 60, self:getDayStreak()))
          DailyReward.insertHistory(self:getGuid(), self:getDayStreak(), "Claimed reward no. " .. self:getDayStreak() + 1 .. ". Picked reward: XP Bonus for " .. reward.xpminutes .. " minutes.")
          DailyReward.pickedReward(playerId)
          self:sendDailyRewardOldClient()
        end)
        window:addButton('Back', function(button, choice)
          self:sendDailyRewardOldClient()
        end)
        window:sendToPlayer(self)
        return false
      ...
      -- prey bonus
      if (dailyTable.type == DAILY_REWARD_TYPE_PREY_REROLL) then
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You have claimed %d bonus rerolls on %d day streak.", reward.rerollCount, self:getDayStreak()))
        self:addBonusReroll(reward.rerollCount)
        DailyReward.insertHistory(self:getGuid(), self:getDayStreak(), "Claimed reward no. " .. self:getDayStreak() + 1 .. ". Picked reward: " .. reward.rerollCount .. "x Prey bonus reroll(s)")
        DailyReward.pickedReward(playerId)
        return false
    ...
    elseif (choice == history) then
      local title = "Reward Wall"
      local message = ""
      local entries = DailyReward.retrieveHistoryEntries(self:getId())
      if #entries == 0 then
        message = "You don't have any entries yet."
      else
        message = "Your latest daily reward entries: \n"
        for k, entry in ipairs(entries) do
          if (k ~= #entries) then
            message = message .. os.date('%d-%b-%Y', entry.timestamp) .. ": " .. entry.description .. "\n"
          else
            message = message .. os.date('%d-%b-%Y', entry.timestamp) .. ": " .. entry.description
          end
        end
      ...
      local historywindow = ModalWindow(1000, title, message)
      historywindow:addButton(100, 'Back', function(button, choice) self:sendDailyRewardOldClient() end)
      historywindow:setDefaultEnterButton(100)
      historywindow:sendToPlayer(self)
      end
  end)
