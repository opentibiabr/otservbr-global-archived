...
function forgottenKnowledgeOldDesk.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValue(Storage.ForgottenKnowledge.SilverKey) >= 1 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You already got an old silver key.')
		return true
	end
	if player:getStorageValue(Storage.ForgottenKnowledge.GirlPicture) >= 1 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You open the drawer again and find an old silver key that surely wasn\'t there before..')
	...
	if player:getStorageValue(Storage.ForgottenKnowledge.OldDesk) >= 1 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You already got an old note.')
		return true
	end
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'As you open the drawer a ghostly apparition shortly appears hovering over the desk. You find an old note inside of the drower.')
