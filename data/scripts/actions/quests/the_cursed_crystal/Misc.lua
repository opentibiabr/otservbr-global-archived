...
function theCursedMiscAction.onUse(cid, item, frompos, item2, topos)
	if (item.actionid == 40001) and (item:getPosition() == TCC_SKELETON_PAPER_POS) then -- 35193
		...
		return
	elseif (item.actionid == 40001) and (item:getPosition() == TCC_SKELETON_BELL_POS) then -- 35194
		if (user1:getStorageValue(Storage.TibiaTales.TheCursedCrystal.Oneeyedjoe) < 1) then
