local config = {
	[467] = {requireSoil = false, toPosition = Position(32908, 31081, 7), effect = CONST_ME_ENERGYHIT},
	[14336] = {requireSoil = true, pushbackPosition = Position(32908, 31081, 7), toPosition = Position(32908, 31076, 7), effect = CONST_ME_ENERGYHIT},
	[14337] = {requireSoil = true, pushbackPosition = Position(32906, 31080, 7), toPosition = Position(32908, 31085, 7), effect = CONST_ME_HITBYFIRE},
	[922] = {requireSoil = false, toPosition = Position(32906, 31080, 7), effect = CONST_ME_HITBYFIRE}
}

local magicianQuarter = MoveEvent()

function magicianQuarter.onStepIn(creature, item, position, fromPosition)
	local player = creature:getPlayer()
	if not player then
		return true
	end

	local targetWall = config[fromPosition.y]
	if not targetWall then
		return true
	end

	if targetWall.requireSoil then
		if not (player:removeItem(940, 1) or player:removeItem(941, 1) or player:removeItem(944, 1) or player:removeItem(945, 1)) then
			player:teleportTo(targetWall.pushbackPosition)
			player:say('You may not enter without a sacrifice of elemental soil.', TALKTYPE_MONSTER_SAY)
			targetWall.pushbackPosition:sendMagicEffect(targetWall.effect)
			return true
		end
	end

	player:teleportTo(targetWall.toPosition)
	targetWall.toPosition:sendMagicEffect(targetWall.effect)
	return true
end

magicianQuarter:type("stepin")
magicianQuarter:aid(7813)
magicianQuarter:register()
