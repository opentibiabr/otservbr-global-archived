...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'fafnar fire') then
		if player:getStorageValue(StorageTheShatteredIsles.RaysMission1) == 1 then
	...
	elseif msgcontains(msg, 'your continued existence is payment enough') then
		if npcHandler.topic[cid] == 6 then
			if player:getStorageValue(StorageTheShatteredIsles.RaysMission1) == 1 then
				npcHandler:say(
					'What?? How dare you?! I am a sorcerer of the most reknown academy on the face of this world. Do you think some lousy pirates could scare me? Get lost! Now! I will have no further dealings with the likes of you!',
					cid
				)
				player:setStorageValue(StorageTheShatteredIsles.RaysMission1, 2)
	...
	elseif msgcontains(msg, 'yes') then
		...
		elseif npcHandler.topic[cid] == 2 then
			npcHandler:say(
				"Great! I've signed you up for our bonus system. From now on, you will have the chance to win the potion belt addon!",
				cid
			)
		...
		elseif npcHandler.topic[cid] == 3 then
			if
				player:getStorageValue(38412) >= 100 or player:removeItem(7634, 100) or player:removeItem(7635, 100) or
					player:removeItem(7636, 100)
			 then
				npcHandler:say(
					'Alright, thank you very much! Here is your lottery ticket, good luck. Would you like to deposit more vials that way?',
					cid
				)
				...
				npcHandler:say(
					"Sorry, but you don't have 100 empty flasks or vials of the SAME kind and thus don't qualify for the lottery. Would you like to deposit the vials you have as usual and receive 5 gold per vial?",
					cid
				)
				npcHandler.topic[cid] = 0
			end
		...
		elseif npcHandler.topic[cid] == 5 then
			if player:getStorageValue(StorageTheShatteredIsles.RaysMission1) == 1 then
				npcHandler:say(
					'Finally. You have no idea how difficult it is to keep something secret here. And you brought me all the crystal coins I demanded?',
					cid
				)
...
keywordHandler:addKeyword(
	{'shop'},
	StdModule.say,
	{npcHandler = npcHandler, text = "I sell potions and fluids. If you'd like to see my offers, ask me for a {trade}."}
)
...
npcHandler:setMessage(
	MESSAGE_SENDTRADE,
	"Of course, just browse through my wares. By the way, if you'd like to join our bonus system for depositing flasks and vial, you have to tell me about that {deposit}."
)
