...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "mission") then
		if player:getStorageValue(Storage.TheShatteredIsles.MerianaQuest) == 29 then
		...
		player:setStorageValue(Storage.TheShatteredIsles.MerianaQuest, 30)
		npcHandler.topic[cid] = 0
	elseif(player:getStorageValue(Storage.TheShatteredIsles.MerianaQuest) == 31) and player:getItemCount(11427) == 1 then
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			...
			player:removeItem(11427, 1)
			player:setStorageValue(Storage.TheShatteredIsles.MerianaQuest, 32)
			npcHandler.topic[cid] = 0
		end
	end
	return true
