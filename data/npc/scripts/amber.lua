...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'addon') or msgcontains(msg, 'outfit')
		...
		if addonProgress < 1 then
			npcHandler:say('Sorry, the backpack I wear is not for sale. It\'s \z
			handmade from rare minotaur leather.', cid)
			npcHandler.topic[cid] = 1
		elseif addonProgress == 1 then
			npcHandler:say('Ah, right, almost forgot about the backpack! Have you brought me 100 pieces of \z
			minotaur leather as requested?', cid)
		...
		elseif addonProgress == 2 then
			if player:getStorageValue(Storage.OutfitQuest.Citizen.AddonBackpackTimer) < os.time() then
				...
				npcHandler:say('Uh... I didn\'t expect you to return that early. Sorry, \z
				but I\'m not finished yet with your backpack. I\'m doing the best I can, promised.', cid)
			end
		elseif addonProgress == 3 then
			npcHandler:say('Sorry, but I can only make one backpack per person, else I\'d have to \z
			close my shop and open a leather manufactory.', cid)
		end
		return true
	end
	...
	if npcHandler.topic[cid] == 1 then
		if msgcontains(msg, 'backpack') or msgcontains(msg, 'minotaur') or msgcontains(msg, 'leather') then
			npcHandler:say('Well, if you really like this backpack, I could make one for you, but minotaur \z
			leather is hard to come by these days. Are you willing to put some work into this?', cid)
	...
	elseif npcHandler.topic[cid] == 2 then
		if msgcontains(msg, 'yes') then
			...
			npcHandler:say('Alright then, if you bring me 100 pieces of fine minotaur leather \z
			I will see what I can do for you. You probably have to kill \z
			really many minotaurs though... so good luck!', cid)
	...
	elseif npcHandler.topic[cid] == 3 then
		if msgcontains(msg, 'yes') then
			if player:getItemCount(5878) < 100 then
				...
				npcHandler:say('Great! Alright, I need a while to finish this backpack for you. \z
				Come ask me later, okay?', cid)
			...
			npcHandler:say('I know, it\'s quite some work... don\'t lose heart, just keep killing minotaurs and you\'ll \z
			eventually get lucky. Would you rather like to buy a normal backpack for 10 gold?', cid)
		end
		npcHandler.topic[cid] = 0
	end
end
...
-- Basic keywords
keywordHandler:addKeyword({'hint'}, StdModule.rookgaardHints, 
	{
...
keywordHandler:addKeyword({'adventure'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'I fought fierce {monsters}, climbed the highest mountains, explored the deepest \z
		{dungeons} and crossed the {sea} on a {raft}.'
	}
)
keywordHandler:addKeyword({'sea'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'My voyage on the sea was exhausting. The weather was bad, the waves high and my raft quite simple. \z
...
keywordHandler:addKeyword({'help'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Well, I can give you general {hints} or tell you about my {adventures} \z
		and many other topics. Oh, and if you are bored, I might have a small {quest} for you.'
	}
)
...
keywordHandler:addKeyword({'dungeon'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'I had no time to explore the dungeons of this isle yet, but I\'ve seen two big caves in the East, \z
		and there is a ruined tower to the north-west. Oh, and there\'re the {sewers}.'
	}
)
...
keywordHandler:addKeyword({'dragon'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Their breath is so hot! I had to cut my hair after my last encounter with a dragon because the \z
		ends were all burnt. That\'s what you gotta deal with as a female adventurer!'
	}
)
...
keywordHandler:addKeyword({'weapon'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'The best weapons on this isle are just toothpicks compared with the weapons \z
		warriors wield on the {mainland}.'
	}
)
...
keywordHandler:addKeyword({'zirella'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'She seriously asked me if she could have the remains of my {raft} as fire wood! \z
...
keywordHandler:addKeyword({'billy'}, StdModule.say, 
	{
		...
		but I simply can\'t eat something like that. So I told him I\'m a vegetarian and \z
		I only eat fish. <gulps>'
	}
)
...
salmonKeyword:addChildKeyword({'no'}, StdModule.say, 
	{	npcHandler = npcHandler, 
...
bookKeyword:addChildKeyword({'no'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Too bad.', 
		reset = true
