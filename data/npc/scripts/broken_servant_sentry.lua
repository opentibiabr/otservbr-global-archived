...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "cap") or msgcontains(msg, "mage")) then
		if(getPlayerItemCount(cid, 12599) >= 1 and getPlayerStorageValue(cid, Storage.ElementalistQuest1) == 2) and getPlayerStorageValue(cid, Storage.ElementalistQuest2) < 1 then
			selfSay("Yo-Yo-Your*chhhrk*. Cap. Is. Slimed. I. Can. Clean. It. *chhhhrrrkchrk* ...", cid)
			npcHandler:say("Here. You. Are. *chhhrrrrkchrk*", cid)
			doPlayerRemoveItem(cid, 12599, 1)
			setPlayerStorageValue(cid, Storage.ElementalistQuest2, 1)
			doPlayerAddOutfit(cid, 437, 1)
			doPlayerAddOutfit(cid, 438, 1)
	...
	elseif(msgcontains(msg, "staff") or msgcontains(msg, "spike")) then
		if(getPlayerItemCount(cid, 12803) >= 1 and getPlayerStorageValue(cid, Storage.ElementalistQuest1) == 2) and getPlayerStorageValue(cid, Storage.ElementalistQuest3) < 1 then
			npcHandler:say({"Yo-Yo-Your*chhhrk*. Cap. Is. Slimed. I. Can. Clean. It. *chhhhrrrkchrk* ...",
				"Here. You. Are. *chhhrrrrkchrk*"}, cid, 0, 1, 4000)
			doPlayerRemoveItem(cid, 12803, 1)
			setPlayerStorageValue(cid, Storage.ElementalistQuest3, 1)
			doPlayerAddOutfit(cid, 437, 2)
			doPlayerAddOutfit(cid, 438, 2)
			npcHandler.topic[cid] = 0
	...
	elseif(msgcontains(msg, "yes")) then
		...
		elseif(npcHandler.topic[cid] == 4) and getPlayerItemCount(cid, 12601) >= 20 then
				...
				doPlayerRemoveItem(cid, 12601, 20)
				setPlayerStorageValue(cid, Storage.ElementalistQuest1, 2)
				doPlayerAddOutfit(cid, 437, 0)
				doPlayerAddOutfit(cid, 438, 0)
				npcHandler.topic[cid] = 0
			else
