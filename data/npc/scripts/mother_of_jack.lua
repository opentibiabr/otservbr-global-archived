...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "jack") then
		if (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.QuestLine) == 5) then
			if (player:getStorageValue(Storage.TibiaTales.JackFutureQuest.Mother) < 1) then
				npcHandler:say(
					"What about him? He's downstairs as he always has been. He never went away from home any further than into the forest nearby. He rarely ever took a walk to Edron, did he?",
					cid
				)
	...
	elseif msgcontains(msg, "no") then
		if (npcHandler.topic[cid] == 2) then
			npcHandler:say(
				"Thought so. Of course he wouldn't do anything wrong. And he went where? Edron. Hm. I can see nothing wrong with that. But... he wasn't there often, was he?",
				cid
			)
