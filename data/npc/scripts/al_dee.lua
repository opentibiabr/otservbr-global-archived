...
shopModule:addBuyableItem({"scythe"}, 2550, 12, 1)
...
keywordHandler:addKeyword({'help'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'If you need general {equipment}, just ask me for a {trade}. \z
		I can also provide you with some general {hints} about the game.'
	}
)
keywordHandler:addKeyword({'equip'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'As an adventurer, you should always have at least a {backpack}, a {rope}, a {shovel}, \z
		a {weapon}, an {armor} and a {shield}.'
	}
)
...
keywordHandler:addKeyword({'monster'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'If you want to challenge monsters in the {dungeons}, you need some {weapons} and {armor} \z
...
keywordHandler:addKeyword({'dungeon'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'If you want to explore the dungeons such as the {sewers}, you have to {equip} yourself with the \z
		vital stuff I am selling. It\'s {vital} in the deepest sense of the word.'})
keywordHandler:addKeyword({'sewer'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Oh, our sewer system is very primitive - it\'s so primitive that it\'s overrun by {rats}. \z
		But the stuff I sell is safe from them. Just ask me for a {trade} to see it!'
	}
)
...
keywordHandler:addKeyword({'mainland'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Have you ever wondered what that \'main\' is people are talking about? Well, once you\'ve \z
		reached level 8, you should talk to the {oracle}. You can choose a {profession} afterwards and explore \z
		much more of Tibia.'
	}
)
keywordHandler:addKeyword({'weapon'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Oh, I\'m sorry, but I don\'t deal with weapons. That\'s {Obi\'s} or {Lee\'Delle\'s} business. \z
		I could offer you a {pick} in exchange for a {small axe} if you should happen to own one.'
	}
)
...
keywordHandler:addKeyword({'blueberr'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Blueberries grow on bushes. They\'re quite common in Tibia. Just pick them from a bush if \z
		you need a snack!'
	}
)
...
keywordHandler:addKeyword({'food'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Hmm, the best address to look for food might be {Willie} or {Billy}. {Norma} also \z
		has some snacks for sale.'
	}
)
keywordHandler:addKeyword({'bank'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'A bank is quite useful. You can deposit your money safely there. This way you don\'t \z
		have to carry it around with you all the time. You could also invest your money in my {wares}!'
	}
)
keywordHandler:addKeyword({'academy'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'The big building in the centre of Rookgaard. They have a library, a training centre, \z
		a {bank} and the room of the {oracle}. {Seymour} is the teacher there.'
	}
)
...
keywordHandler:addKeyword({'merchant'}, StdModule.say, 
	{
		npcHandler = npcHandler, text = 'To view the offers of a merchant, simply talk to him \z
		or her and ask for a {trade}. They will gladly show you their offers and also the things they buy from you.'
	}
)
keywordHandler:addKeyword({'profession'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'You will learn everything you need to know about professions once you\'ve reached \z
...
keywordHandler:addKeyword({'destiny'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'The Island of Destiny can be reached via the {oracle} once you are level 8. \z
		This trip will help you choose your {profession}!'
	}
)
...
keywordHandler:addKeyword({'dixi'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'She\'s {Obi\'s} granddaughter and deals with {armors} and {shields}. \z
		Her shop is south west of town, close to the {temple}.'
	}
)
...
keywordHandler:addKeyword({'lee\'delle'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'If you are a {premium} adventurer, you should check out {Lee\'Delle\'s} shop. \z
		She lives in the western part of town, just across the bridge.'
	}
)
...
keywordHandler:addKeyword({'oracle'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'You can find the oracle on the top floor of the {academy}, just above {Seymour}. \z
		Go there when you are level 8.'
	}
)
...
keywordHandler:addKeyword({'lily'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'She sells health {potions} and antidote potions. Also, she buys {blueberries} and {cookies} \z
...
keywordHandler:addKeyword({'willie'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'This is a local farmer. If you need fresh {food} to regain your health, it\'s a good place to go. \z
		However, many monsters also carry food such as meat or cheese. Or you could simply pick {blueberries}.'
	}
)
keywordHandler:addKeyword({'billy'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'This is a local farmer. If you need fresh {food} to regain your health, it\'s a good place to go. \z
		He\'s only trading with {premium} adventurers though.'
	}
)
keywordHandler:addKeyword({'norma'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'She used to sell equipment, but I think she has opened a small bar now. \z
		Talks about changing her name to \'Mary\' and such, strange girl.'
	}
)
...
local pickKeyword = keywordHandler:addKeyword({'pick'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Picks are hard to come by. I trade them only in exchange for high quality small axes. \z
		Would you like to make that deal?'
	}
)
pickKeyword:addChildKeyword({'yes'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Splendid! Here, take your pick.', reset = true
	},
...
pickKeyword:addChildKeyword({'yes'}, StdModule.say, 
		{
			npcHandler = npcHandler, 
			text = 'Sorry, I am looking for a SMALL axe.', reset = true
		}
)
pickKeyword:addChildKeyword({'no'}, StdModule.say, 
	{
		npcHandler = npcHandler, 
		text = 'Well, then don\'t.', reset = true
	}
)
...
npcHandler:setMessage(MESSAGE_GREET, {
	'Hello, hello, |PLAYERNAME|! Please come in, look, and buy! I\'m a specialist for all sorts of {tools}. \z
	Just ask me for a {trade} to see my offers! You can also ask me for general {hints} about the game. ...',
