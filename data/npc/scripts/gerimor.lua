...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "missions") then
		...
		elseif player:getStorageValue(Storage.CultsOfTibia.FinalBoss.Mission) == 2 then
			...
			local item = ""
			if player:getVocation():getVocationBaseId() == VOCATION.BASE_ID.SORCERER then
				player:addItem(29426)
				item = "reflecting crown"
			end
			if player:getVocation():getVocationBaseId() == VOCATION.BASE_ID.DRUID then
				player:addItem(29423)
				item = "leaf crown"
			end
			if player:getVocation():getVocationBaseId() == VOCATION.BASE_ID.PALADIN then
				player:addItem(29425)
				item = "incandescent crown"
			end
			if player:getVocation():getVocationBaseId() == VOCATION.BASE_ID.KNIGHT then
