...
function dreamFirst(cid, message, keywords, parameters, node)
	...
	if isPremium(cid) then
		...
		if addon == -1 then
			if getPlayerItemCount(cid,20276) >= 1 then
				if doPlayerRemoveItem(cid,20276,1) then
...
function dreamSecond(cid, message, keywords, parameters, node)
	...
	if isPremium(cid) then
		...
		if addon == -1 then
			if getPlayerItemCount(cid,20275) >= 1 then
				if doPlayerRemoveItem(cid,20275,1) then
					selfSay(newaddon, cid)
