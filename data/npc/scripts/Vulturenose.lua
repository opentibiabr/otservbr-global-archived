...
if msgcontains(msg, 'enter') then
		if player:getStorageValue(Storage.TheShatteredIsles.MerianaQuest) >= 27 then
			...
			if headItem and headItem.itemid == 6096 and
			   armorItem and armorItem.itemid == 6095 and
			   legsItem and legsItem.itemid == 5918 and
			   feetItem and feetItem.itemid == 5462 then
			   npcHandler:say("Hey, I rarely see a dashing pirate like you! Get in, matey!", cid)
			   player:setStorageValue(Storage.TheShatteredIsles.DoorPirateSet, 1)
			else
...
npcHandler:setMessage(MESSAGE_GREET, 'Hrrrrm')
