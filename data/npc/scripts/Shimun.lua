...
local player = Player(cid)
if msgcontains(msg, "mission") and player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 1 then
	if player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 1 then
		npcHandler:say({"Could you help me do a ritual?"}, cid)-- It needs to be revised, it's not the same as the global
		npcHandler.topic[cid] = 1
		playerTopic[cid] = 1
	end
elseif msgcontains(msg, "yes") and playerTopic[cid] == 1 and player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 1 then
	if player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 1 then	
		...
		player:setStorageValue(Storage.KilmareshQuest.Eighth.Shimun, 2)
...
elseif msgcontains(msg, "yes") and playerTopic[cid] == 3 and player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 2 then
	if player:getStorageValue(Storage.KilmareshQuest.Eighth.Shimun) == 2 and player:getItemById(36175, 1) and player:getItemById(36171, 12) and player:getItemById(2006, 5) then	
		...
		player:setStorageValue(Storage.KilmareshQuest.Eighth.Shimun, 3)
