...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.actionid == 12550 or target.actionid == 12551 then -- Secret Service Quest
	...
	elseif target.actionid == 12718 or target.actionid == 12719 then -- Meriana Quest
		if player:getStorageValue(Storage.TheShatteredIsles.MerianaQuest) ~= 24 then
			toPosition:sendMagicEffect(CONST_ME_FIREAREA)
			player:setStorageValue(Storage.TheShatteredIsles.MerianaQuest, 25)
			end
		end
	return false
end
