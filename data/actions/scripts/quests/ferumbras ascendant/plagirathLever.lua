...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		if player:getStorageValue(Storage.FerumbrasAscension.PlagirathTimer) >= 1 then
		...
		for y = 31500, 31504 do
			...
			if playerTile and playerTile:isPlayer() then
				...
				playerTile:setExhaustion(Storage.FerumbrasAscension.PlagirathTimer, 60 * 60 * 2 * 24)
			end
		end
		Game.setStorageValue(GlobalStorage.FerumbrasAscendantQuest.PlagirathTimer, 1)
