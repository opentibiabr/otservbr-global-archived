...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		if player:getStorageValue(Storage.FerumbrasAscension.ShulgraxTimer) >= 1 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need to wait a while, recently someone challenge Shulgrax.")
			return true
		end
		...
		for y = 32785, 32789 do
			...
			if playerTile and playerTile:isPlayer() then
				...
				playerTile:setExhaustion(Storage.FerumbrasAscension.ShulgraxTimer, 60 * 60 * 2 * 24)
			end
		end
		Game.setStorageValue(GlobalStorage.FerumbrasAscendantQuest.ShulgraxTimer, 1)
