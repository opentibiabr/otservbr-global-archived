...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		if player:getStorageValue(Storage.FerumbrasAscension.TheShattererTimer) >= 1 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need to wait a while, recently someone challenge The Shatterer.")
			return true
		end
		...
		for x = 33403, 33407 do
			...
			if playerTile and playerTile:isPlayer() then
				...
				playerTile:setExhaustion(Storage.FerumbrasAscension.TheShattererTimer, 60 * 60 * 2 * 24)
			end
		end
