...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		if player:getStorageValue(Storage.FerumbrasAscension.ZamuloshTimer) >= 1 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need to wait a while, recently someone challenge Zamulosh.")
			return true
		end
		...
		for y = 32741, 32745 do
			...
			if playerTile and playerTile:isPlayer() then
				...
				playerTile:setExhaustion(Storage.FerumbrasAscension.ZamuloshTimer, 60 * 60 * 2 * 24)
			end
		end
