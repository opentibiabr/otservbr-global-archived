function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 10029 then
		if Game.getStorageValue(GlobalStorage.FerumbrasAscendantQuest.Elements.Done) < 2 then
		...
		if Game.getStorageValue(GlobalStorage.FerumbrasAscendantQuest.Elements.Done) >= 2 then
			Game.setStorageValue(GlobalStorage.FerumbrasAscendantQuest.Elements.Done, 3)
