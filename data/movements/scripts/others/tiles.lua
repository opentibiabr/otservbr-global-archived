...
function onStepIn(creature, item, position, fromPosition)
	...
	if position:getTile():hasFlag(TILESTATE_PROTECTIONZONE) then
		...
		if depotItem ~= nil then
			...
			player:sendTextMessage(MESSAGE_STATUS_DEFAULT, "Your depot contains " .. depotItems .. " item" .. (depotItems > 1 and "s." or ".") .. " Your supply stash contains " .. player:getStashCount() .. " item" .. (player:getStashCount()	 > 1 and "s." or "."))
