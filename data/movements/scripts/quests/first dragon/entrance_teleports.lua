...
function onStepIn(creature, item, position, fromPosition)
	...
	if player:getStorageValue(Storage.FirstDragon.Questline) < 1 or player:getStorageValue(teleport.storage) < teleport.value then
		position:sendMagicEffect(CONST_ME_TELEPORT)
		player:teleportTo(fromPosition)
		player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
		player:say('You don\'t have permission to use this portal', TALKTYPE_MONSTER_SAY)
		return true
	elseif player:getStorageValue(teleport.storage) >= teleport.value then
		position:sendMagicEffect(CONST_ME_TELEPORT)
		...
		addEvent(clearBossRoom, 60 * 10 * 1000, player.uid, monster.uid, teleport.bossPos, teleport.range, fromPosition)
		return true
	end
