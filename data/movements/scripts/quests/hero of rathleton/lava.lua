...
local function checkCounter()
	local w = Game.getStorageValue(GlobalStorage.HeroRathleton.LavaChange)
	if w == 7 then
		local spectators = Game.getSpectators(changes[w].centerPos, false, false, 13, 13, 13, 13)
	...
	for i = 1, #spectators do
		local spectator = spectators[i]
		if spectator:isPlayer() then
			spectator:teleportTo(changes[w].nextPos)
		end
	end
	if w < 7 then
		for p = 1, 5 do
			addEvent(Game.createMonster, 15 * 1000, 'raging fire', changes[w].nextPos, true, true)
		end
	end
...
function onStepIn(creature, item, position, fromPosition)
	...
	if Game.getStorageValue(GlobalStorage.HeroRathleton.LavaRunning) == 1 then
		player:say("Has someone trying active the machine.\nTry again later.", TALKTYPE_MONSTER_SAY, false, nil, position)
		player:teleportTo(Position(33371, 31955, 15))
		position:sendMagicEffect(CONST_ME_TELEPORT)
		player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
		return true
	end
	if Game.getStorageValue(GlobalStorage.HeroRathleton.FourthMachines) < 8 then
