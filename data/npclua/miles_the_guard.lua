...
local function creatureSayCallback(npc, creature, type, message)
	...
	if msgcontains(message, "trouble") and npcHandler.topic[creature] ~= 3 and player:getStorageValue(Storage.TheInquisition.MilesGuard) < 1 and player:getStorageValue(Storage.TheInquisition.Mission01) ~= -1 then
		npcHandler:say("I'm fine. There's no trouble at all.", npc, creature)
		npcHandler.topic[creature] = 1
	elseif msgcontains(message, "foresight of the authorities") and npcHandler.topic[creature] == 1 then
		npcHandler:say("Well, of course. We live in safety and peace.", npc, creature)
		npcHandler.topic[creature] = 2
	elseif msgcontains(message, "also for the gods") and npcHandler.topic[creature] == 2 then
		npcHandler:say("I think the gods are looking after us and their hands shield us from evil.", npc, creature)
		npcHandler.topic[creature] = 3
	elseif msgcontains(message, "trouble will arise in the near future") and npcHandler.topic[creature] == 3 then
