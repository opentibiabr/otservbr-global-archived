...
bool Weapons::registerEvent(Event_ptr event, const pugi::xml_node&) {
  ...
  if (!result.second) {
    std::cout << "[Warning - Weapons::registerEvent] Duplicate registered item "
                 "with id: "
...
bool Weapon::configureEvent(const pugi::xml_node& node) {
  ...
  if (!(attr = node.attribute("id"))) {
    std::cout << "[Error - Weapon::configureEvent] Weapon without id."
              << std::endl;
    return false;
  }
  ...
  if ((attr = node.attribute("breakchance")) &&
      g_config.getBoolean(ConfigManager::REMOVE_WEAPON_CHARGES)) {
  ...
  if ((attr = node.attribute("action"))) {
    ...
    if (action == WEAPONACTION_NONE) {
      std::cout << "[Warning - Weapon::configureEvent] Unknown action "
...
void Weapon::onUsedWeapon(Player* player, Item* item, Tile* destTile) const {
  ...
  switch (action) {
    ...
    case WEAPONACTION_MOVE:
      g_game.internalMoveItem(item->getParent(), destTile, INDEX_WHEREVER,
...
bool Weapon::executeUseWeapon(Player* player, const LuaVariant& var) const {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - Weapon::executeUseWeapon] Call stack overflow"
              << std::endl;
    return false;
  }
  ...
  if (chance >= uniform_random(1, 100)) {
    ...
    if (!Position::areInRange<1, 1, 0>(player->getPosition(),
      ...
      for (const auto& dir : destList) {
        ...
        Tile* tmpTile = g_game.map.getTile(destPos.x + dir.first,
                                           destPos.y + dir.second, destPos.z);
...
bool WeaponWand::configureEvent(const pugi::xml_node& node) {
  ...
  if (tmpStrValue == "earth") {
    ...
    std::cout << "[Warning - WeaponWand::configureEvent] Type \""
