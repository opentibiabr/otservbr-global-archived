...
void Game::start(ServiceManager* manager) {
  ...
  g_scheduler.addEvent(createSchedulerTask(
      EVENT_IMBUEMENTINTERVAL, std::bind(&Game::checkImbuements, this)));
}
    ...
    if (newitemType.stackable) {
      ...
      if (item->getItemCount() < 100 &&
        ...
        while (player->getItemTypeCount(item->getID())) {
          ...
          Container* mainBP =
              player->getInventoryItem(CONST_SLOT_BACKPACK)->getContainer();
...
bool Game::loadMainMap(const std::string& filename) {
  Monster::despawnRange =
      g_config.getNumber(ConfigManager::DEFAULT_DESPAWNRANGE);
  Monster::despawnRadius =
      g_config.getNumber(ConfigManager::DEFAULT_DESPAWNRADIUS);
  return map.loadMap("data/world/" + filename + ".otbm", true, true);
}

void Game::loadMap(const std::string& path) { map.loadMap(path, false, false); }
...
Thing* Game::internalGetThing(Player* player, const Position& pos,
                              int32_t index, uint32_t spriteId,
  ...
  if (pos.y & 0x40) {
    ...
    if (it.isFluidContainer() &&
        index <
            static_cast<int32_t>(sizeof(reverseFluidMap) / sizeof(uint8_t))) {
...
void Game::playerMoveThing(uint32_t playerId, const Position& fromPos,
  ...
  if (Creature* movingCreature = thing->getCreature()) {
    ...
    playerMoveItem(player, fromPos, spriteId, fromStackPos, toPos, count,
...
void Game::playerMoveCreature(Player* player, Creature* movingCreature,
  ...
  if (player != movingCreature) {
    if (toTile->hasFlag(TILESTATE_BLOCKPATH)) {
      ...
      if (movingNpc && !Spawns::isInZone(movingNpc->getMasterPos(),
                                         movingNpc->getMasterRadius(), toPos)) {
        player->sendCancelMessage(RETURNVALUE_NOTENOUGHROOM);
        return;
      }
    }
...
void Game::playerMoveItemByPlayerID(uint32_t playerId, const Position& fromPos,
  ...
  playerMoveItem(player, fromPos, spriteId, fromStackPos, toPos, count, nullptr,
                 nullptr);
}
...
void Game::playerMoveItem(Player* player, const Position& fromPos,
  ...
  if (item == nullptr) {
    ...
    Thing* thing =
        internalGetThing(player, fromPos, fromIndex, 0, STACKPOS_MOVE);
    if (!thing || !thing->getItem()) {
      player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
      return;
    }
    ...
    if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos)) {
      ...
      if (fromPos.x != 0xFFFF &&
        ...
        ReturnValue ret = internalMoveItem(
            fromCylinder, player, INDEX_WHEREVER, item, count, &moveItem);
        if (ret != RETURNVALUE_NOERROR) {
          player->sendCancelMessage(ret);
          return;
        }
      ...
      if (player->getPathTo(walkPos, listDir, 0, 0, true, true)) {
        ...
        SchedulerTask* task = createSchedulerTask(
            ...
            std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(),
                      itemPos, spriteId, itemStackPos, toPos, count));
        player->setNextWalkActionTask(task);
      } else {
        player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
      }
...
ReturnValue Game::internalMoveItem(Cylinder* fromCylinder, Cylinder* toCylinder,
                                   ...
                                   Item** _moveItem, uint32_t flags /*= 0*/,
  ...
  if (ret == RETURNVALUE_NEEDEXCHANGE) {
    ...
    if (ret == RETURNVALUE_NOERROR) {
      ...
      ReturnValue retExchangeMaxCount = fromCylinder->queryMaxCount(
          INDEX_WHEREVER, *toItem, toItem->getItemCount(),
...
ReturnValue Game::internalAddItem(Cylinder* toCylinder, Item* item,
  ...
  */
  uint32_t maxQueryCount = 0;
  ret = destCylinder->queryMaxCount(INDEX_WHEREVER, *item,
  ...
  if (item->isStackable() && item->equals(toItem)) {
    ...
    if (count > 0) {
      if (item->getItemCount() != count) {
        ...
        if (internalAddItem(destCylinder, remainderItem, INDEX_WHEREVER, flags,
...
ReturnValue Game::internalPlayerAddItem(
    Player* player, Item* item, bool dropOnMap /*= true*/,
  ...
  if (ret != RETURNVALUE_NOERROR && dropOnMap) {
    ret =
        internalAddItem(player->getTile(), item, INDEX_WHEREVER, FLAG_NOLIMIT);
  }
...
void Game::addMoney(Cylinder* cylinder, uint64_t money,
  ...
  while (crystalCoins > 0) {
    ...
    ReturnValue ret =
        internalAddItem(cylinder, remaindItem, INDEX_WHEREVER, flags);
    if (ret != RETURNVALUE_NOERROR) {
      internalAddItem(cylinder->getTile(), remaindItem, INDEX_WHEREVER,
                      FLAG_NOLIMIT);
    }
...
ReturnValue Game::internalTeleport(Thing* thing, const Position& newPos,
  ...
  if (Creature* creature = thing->getCreature()) {
    ...
    return internalMoveItem(item->getParent(), toTile, INDEX_WHEREVER, item,
...
ReturnValue Game::internalQuickLootItem(
  ...
  do {
    ...
    if (item->getParent()) {  // Stash retrive dont have parent cylinder.
      ret = internalMoveItem(item->getParent(), lootContainer, INDEX_WHEREVER,
                             item, item->getItemCount(), &moveItem, 0, player);
    } else {
      ret = internalAddItem(lootContainer, item, INDEX_WHEREVER);
    }
    if (moveItem) {
...
bool Game::playerBroadcastMessage(Player* player,
  ...
  std::cout << "> " << player->getName() << " broadcasted: \"" << text << "\"."
            << std::endl;
...
void Game::playerUseItemEx(uint32_t playerId, const Position& fromPos,
                           uint8_t fromStackPos, uint16_t fromSpriteId,
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (fromPos.x != 0xFFFF && toPos.x != 0xFFFF &&
        ...
        ret = internalMoveItem(item->getParent(), player, INDEX_WHEREVER, item,
      ...
      if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
        ...
        SchedulerTask* task = createSchedulerTask(
            400, std::bind(&Game::playerUseItemEx, this, playerId, itemPos,
                           itemStackPos, fromSpriteId, toPos, toStackPos,
                           toSpriteId));
        if (it.isRune() || it.type == ITEM_TYPE_POTION) {
...
void Game::playerUseItem(uint32_t playerId, const Position& pos,
                         uint8_t stackPos, uint8_t index, uint16_t spriteId) {
  Player* player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  Thing* thing =
      internalGetThing(player, pos, stackPos, spriteId, STACKPOS_FIND_THING);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
...
void Game::playerUseWithCreature(uint32_t playerId, const Position& fromPos,
  ...
  Thing* thing = internalGetThing(player, fromPos, fromStackPos, spriteId,
                                  STACKPOS_FIND_THING);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid) {
  ...
  if (!parentContainer) {
    ...
    if (it == browseFields.end()) {
      ...
      g_scheduler.addEvent(createSchedulerTask(
          30000,
          std::bind(&Game::decreaseBrowseFieldRef, this, tile->getPosition())));
    } else {
...
void Game::playerRotateItem(uint32_t playerId, const Position& pos,
  ...
  Thing* thing =
      internalGetThing(player, pos, stackPos, 0, STACKPOS_TOPDOWN_ITEM);
  if (!thing) {
    return;
  }
  ...
  if (pos.x != 0xFFFF &&
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      SchedulerTask* task = createSchedulerTask(
          400, std::bind(&Game::playerRotateItem, this, playerId, pos, stackPos,
                         spriteId));
      player->setNextWalkActionTask(task);
    } else {
      player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
...
void Game::playerWrapableItem(uint32_t playerId, const Position& pos,
  ...
  if (!item || item->getClientID() != spriteId ||
      item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) ||
      (!item->isWrapable() && item->getID() != 26054)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (pos.x != 0xFFFF &&
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      SchedulerTask* task = createSchedulerTask(
          400, std::bind(&Game::playerWrapableItem, this, playerId, pos,
                         stackPos, spriteId));
      player->setNextWalkActionTask(task);
    } else {
      player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
  ...
  if ((item->getHoldingPlayer() && item->getID() == 26054) ||
...
void Game::playerStowItem(Player* player, const Position& pos,
                          uint16_t spriteId, uint8_t stackpos, uint32_t count) {
  if (!player->isPremium()) {
  ...
  Item* item = thing->getItem();
  if (!item || (item->getClientID() != spriteId &&
                static_cast<uint32_t>(item->getItemCount()) >= count)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
...
void Game::playerStowAllItems(Player* player, const Position& pos,
...
void Game::playerStashWithdraw(Player* player, uint16_t spriteId,
...
void Game::playerStowContainer(Player* player, const Position& pos,
                               uint16_t spriteId, uint8_t stackpos) {
  if (!player->isPremium()) {
    player->sendCancelMessage(RETURNVALUE_YOUNEEDPREMIUMACCOUNT);
    return;
...
void Game::playerAcceptTrade(uint32_t playerId) {
  ...
  if (tradePartner->getTradeState() == TRADE_ACCEPT) {
    ...
    ReturnValue ret1 =
        internalAddItem(tradePartner, tradeItem1, INDEX_WHEREVER, 0, true);
    ReturnValue ret2 =
        internalAddItem(player, tradeItem2, INDEX_WHEREVER, 0, true);
    if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR) {
      ...
      if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR) {
        ...
        ret1 = internalMoveItem(cylinder1, tradePartner, INDEX_WHEREVER,
                                tradeItem1, count1, nullptr,
                                FLAG_IGNOREAUTOSTACK, nullptr, tradeItem2);
        if (ret1 == RETURNVALUE_NOERROR) {
          internalMoveItem(cylinder2, player, INDEX_WHEREVER, tradeItem2,
...
void Game::playerPurchaseItem(uint32_t playerId, uint16_t spriteId,
  ...
  merchant->onPlayerTrade(player, onBuy, it.id, subType, amount, ignoreCap,
                          inBackpacks);
}

void Game::playerSellItem(uint32_t playerId, uint16_t spriteId, uint8_t count,
  ...
  merchant->onPlayerTrade(player, onSell, it.id, subType, amount,
                          ignoreEquipped);
}
...
void Game::playerLookInShop(uint32_t playerId, uint16_t spriteId,
                            uint8_t count) {
  Player* player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerLookAt(uint32_t playerId, const Position& pos,
                        uint8_t stackPos) {
  Player* player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerQuickLoot(uint32_t playerId, const Position& pos,
  ...
  if (!player->canDoAction()) {
    uint32_t delay = player->getNextActionTime();
    SchedulerTask* task = createSchedulerTask(
        delay, std::bind(&Game::playerQuickLoot, this, player->getID(), pos,
    ...
    if (!Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
      ...
      if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
        g_dispatcher.addTask(createTask(
            std::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
        SchedulerTask* task = createSchedulerTask(
            0, std::bind(&Game::playerQuickLoot, this, player->getID(), pos,
                         spriteId, stackPos, defaultItem));
        player->setNextWalkActionTask(task);
      } else {
        player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
...
void Game::playerSetLootContainer(uint32_t playerId, ObjectCategory_t category,
                                  const Position& pos, uint16_t spriteId,
                                  uint8_t stackPos) {
  Player* player = getPlayerByID(playerId);
  ...
  Thing* thing =
      internalGetThing(player, pos, stackPos, spriteId, STACKPOS_USEITEM);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
...
void Game::playerOpenLootContainer(uint32_t playerId,
  ...
  player->sendContainer(container->getClientID(), container,
...
void Game::playerApplyImbuement(uint32_t playerId, uint32_t imbuementid,
  ...
  g_events->eventPlayerOnApplyImbuement(player, imbuement, item, slot,
                                        protectionCharm);
}
...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature* target,
  ...
  switch (combatType) {
    case COMBAT_PHYSICALDAMAGE: {
      ...
      if (splash) {
        internalAddItem(target->getTile(), splash, INDEX_WHEREVER,
                        FLAG_NOLIMIT);
        startDecay(splash);
      }
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target,
                              CombatDamage& damage, bool isEvent) {
  using namespace std;
...
void Game::internalDecayItem(Item* item) {
  ...
  if (it.decayTo != 0) {
    ...
    if (ret != RETURNVALUE_NOERROR) {
      std::cout << "[Debug - Game::internalDecayItem] internalDecayItem "
                   "failed, error code: "
                << static_cast<uint32_t>(ret) << ", item id: " << item->getID()
                << std::endl;
    }
  }
}
...
void Game::checkDecay() {
  ...
  while (it != end) {
    ...
    int32_t decreaseTime =
        std::min<int32_t>(EVENT_DECAYINTERVAL * EVENT_DECAY_BUCKETS, duration);
    ...
    if (duration <= 0) {
      ...
      it = decayItems[bucket].erase(it);
      size_t newBucket =
          (bucket + ((duration + EVENT_DECAYINTERVAL / 2) / 1000)) %
          EVENT_DECAY_BUCKETS;
      if (newBucket == bucket) {
        internalDecayItem(item);
        ReleaseItem(item);
...
void Game::checkImbuements() {
  ...
  while (it != end) {
    ...
    for (uint8_t slot = 0; slot < slots; slot++) {
      ...
      int32_t newDuration = std::max(
          0, (duration -
              (EVENT_IMBUEMENTINTERVAL * EVENT_IMBUEMENT_BUCKETS) / 690));
    ...
    for (uint8_t slot = 0; slot < slots; slot++) {
      uint32_t info = item->getImbuement(slot);
      int32_t duration = info >> 8;
      int32_t decreaseTime = std::min<int32_t>(
          (EVENT_IMBUEMENTINTERVAL * EVENT_IMBUEMENT_BUCKETS) / 690, duration);
      duration -= decreaseTime;
...
void Game::cleanup() {
  ...
  for (Item* item : toDecayItems) {
    ...
    if (dur >= EVENT_DECAYINTERVAL * EVENT_DECAY_BUCKETS) {
      ...
      decayItems[(lastBucket + 1 + dur / 1000) % EVENT_DECAY_BUCKETS].push_back(
          item);
    }
  }
  toDecayItems.clear();
...
void Game::loadMotdNum() {
  ...
  if (result) {
    ...
    db.executeQuery(
        "INSERT INTO `server_config` (`config`, `value`) VALUES ('motd_num', "
        "'0')");
  }
  ...
  if (result) {
    ...
    db.executeQuery(
        "INSERT INTO `server_config` (`config`, `value`) VALUES ('motd_hash', "
        "'')");
  }
}
...
void Game::playerCreateMarketOffer(
    uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount,
    uint32_t price, bool anonymous)  // Limit of 64k of items to create offer
{
  ...
  if (g_config.getBoolean(ConfigManager::MARKET_PREMIUM) &&
      !player->isPremium()) {
  ...
  if (type == MARKETACTION_SELL) {
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      if (stashminus > 0) {
        ...
        IOStash::withdrawItem(player->guid, it.wareId,
                              (amount > stashminus ? stashminus : amount));
      }

      std::forward_list<Item*> itemList =
          getMarketItemList(it.wareId, stashmath, depotLocker);
...
void Game::playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp,
                                   uint16_t counter)  // Market cancel offer
{
  Player* player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (offer.type == MARKETACTION_BUY) {
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      while (tmpAmount > 0) {
        ...
        if (internalAddItem(player->getInbox(), item, INDEX_WHEREVER,
...
void Game::playerAcceptMarketOffer(
    uint32_t playerId, uint32_t timestamp, uint16_t counter,
    uint16_t amount)  // Limit of 64k of items to create offer
{
  if (amount == 0 || amount > 64000) {
    return;
  }
  ...
  if (offer.type == MARKETACTION_BUY) {
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      while (tmpAmount > 0) {
        ...
        Item* item = Item::CreateItem(it.id, stackCount);
        if (internalAddItem(buyerPlayer->getInbox(), item, INDEX_WHEREVER,
                            FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
          delete item;
          break;
        }
  ...
  const int32_t marketOfferDuration =
      g_config.getNumber(ConfigManager::MARKET_OFFER_DURATION);
  ...
  IOMarket::appendHistory(offer.playerId, offer.type, offer.itemId, amount,
                          offer.price, offer.timestamp + marketOfferDuration,
                          OFFERSTATE_ACCEPTED);
...
void Game::playerBuyStoreOffer(uint32_t playerId, uint32_t offerId,
  ...
  if (player) {
    ...
    if (offer->type == ITEM || offer->type == STACKABLE_ITEM ||
      ...
      Container* inbox =
          thing->getItem()->getContainer();  // TODO: Not the right way to get
                                             // the storeInbox
      if (!inbox) {
        player->sendStoreError(
            STORE_ERROR_NETWORK,
      ...
      uint32_t requiredSlots = (tmp->type == ITEM || tmp->type == WRAP_ITEM)
                                   ? tmp->count
                                   : (tmp->count % 100)
                                         ? (uint32_t)(tmp->count / 100) + 1
      ...
      if (freeSlots < requiredSlots) {
        ...
        while (pendingCount > 0) {
          ...
          if (offer->type == WRAP_ITEM) {
            item = Item::CreateItem(TRANSFORM_BOX_ID,
          ...
          if (internalAddItem(inbox, item, INDEX_WHEREVER, FLAG_NOLIMIT) !=
              RETURNVALUE_NOERROR) {
            delete item;
            player->sendStoreError(
      ...
      if (productType == SIMPLE) {  // client didn't sent the new name yet,
        ...
        if (db.storeQuery(query.str())) {  // name already in use
          ...
          switch (nameValidation) {
            case INVALID_LENGTH:
              responseMessage =
                  "Your new name must be more than 3 and less than 14 "
                  "characters long.";
              break;
            case INVALID_TOKEN_LENGTH:
              responseMessage =
                  "Every words of your new name must be at least 2 characters "
                  "long.";
              break;
            case INVALID_FORBIDDEN:
              responseMessage =
                  "You're using forbidden words in your new name.";
              break;
            ...
            case VALID:
              responseMessage =
                  "You have successfully changed you name, you must relog to "
                  "see changes.";
              break;
          }
      ...
      for (auto outfit :
        ...
        if (opositeSexOutfit) {
          player->addOutfit(opositeSexOutfit->lookType,
                            0);  // since addons could have different recipes,
                                 // we can't add automatically
        }
      }
...
std::forward_list<Item*> Game::getMarketItemList(uint16_t wareId,
...
bool Game::reload(ReloadTypes_t reloadType) {
  switch (reloadType) {
    ...
    case RELOAD_TYPE_SPELLS: {
      if (!g_spells->reload()) {
        std::cout << "[Error - Game::reload] Failed to reload spells."
                  << std::endl;
        std::terminate();
      }
      return true;
