    ...
    if (result->getNumber<uint16_t>("sale") == 1) {
      ...
      if (itemType.stackable) {
        ...
        while (tmpAmount > 0) {
          ...
          if (g_game.internalAddItem(player->getInbox(), item, INDEX_WHEREVER,
...
void IOMarket::checkExpiredOffers() {
  ...
  int32_t checkExpiredMarketOffersEachMinutes = g_config.getNumber(
      ConfigManager::CHECK_EXPIRED_MARKET_OFFERS_EACH_MINUTES);
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp,
  ...
  const int32_t created =
      timestamp - g_config.getNumber(ConfigManager::MARKET_OFFER_DURATION);
