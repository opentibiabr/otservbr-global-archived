...
void MoveEvents::clearMap(MoveListMap& map, bool fromLua) {
  for (auto it = map.begin(); it != map.end(); ++it) {
    for (int eventType = MOVE_EVENT_STEP_IN; eventType < MOVE_EVENT_LAST;
         ++eventType) {
...
bool MoveEvents::registerEvent(Event_ptr event, const pugi::xml_node& node) {
  MoveEvent_ptr moveEvent{static_cast<MoveEvent*>(
      event.release())};  // event is guaranteed to be a MoveEvent
  ...
  if (eventType == MOVE_EVENT_ADD_ITEM || eventType == MOVE_EVENT_REMOVE_ITEM) {
    ...
    if (tileItemAttribute &&
        pugi::cast<uint16_t>(tileItemAttribute.value()) == 1) {
      switch (eventType) {
  ...
  if ((attr = node.attribute("itemid"))) {
    ...
    addEvent(std::move(*moveEvent), pugi::cast<int32_t>(attr.value()),
             uniqueIdMap);
  ...
  if (moveEvent->getItemIdRange().size() > 0) {
    if (moveEvent->getItemIdRange().size() == 1) {
      ...
      while (++iterId < moveEvent->getItemIdRange().size()) {
        if (moveEvent->getEventType() == MOVE_EVENT_EQUIP) {
          ItemType& it =
              Item::items.getItemType(moveEvent->getItemIdRange().at(iterId));
          it.wieldInfo = moveEvent->getWieldInfo();
          it.minReqLevel = moveEvent->getReqLevel();
          it.minReqMagicLevel = moveEvent->getReqMagLv();
          it.vocationString = moveEvent->getVocationString();
...
void MoveEvents::addEvent(MoveEvent moveEvent, const Position& pos,
                          MovePosListMap& map) {
...
uint32_t MoveEvents::onCreatureMove(Creature* creature, const Tile* tile,
...
std::string MoveEvent::getScriptEventName() const {
  switch (eventType) {
    ...
    case MOVE_EVENT_REMOVE_ITEM:
      return "onRemoveItem";
    default:
      std::cout << "[Error - MoveEvent::getScriptEventName] Invalid event type"
                << std::endl;
      return std::string();
  }
}
...
bool MoveEvent::configureEvent(const pugi::xml_node& node) {
  ...
  if (!eventAttr) {
    std::cout << "[Error - MoveEvent::configureMoveEvent] Missing event"
              << std::endl;
    return false;
  }
  ...
  if (eventType == MOVE_EVENT_EQUIP || eventType == MOVE_EVENT_DEEQUIP) {
    ...
    for (auto vocationNode : node.children()) {
      pugi::xml_attribute vocationNameAttribute =
          vocationNode.attribute("name");
      if (!vocationNameAttribute) {
        continue;
      }

      int32_t vocationId =
          g_vocations.getVocationId(vocationNameAttribute.as_string());
      if (vocationId != -1) {
        vocEquipMap[vocationId] = true;
        if (vocationNode.attribute("showInDescription").as_bool(true)) {
          vocStringList.push_back(
              asLowerCaseString(vocationNameAttribute.as_string()));
        }
      }
    }
...
bool MoveEvent::loadFunction(const pugi::xml_attribute& attr, bool isScripted) {
  ...
  if (strcasecmp(functionName, "onstepinfield") == 0) {
    ...
    if (!isScripted) {
      std::cout << "[Warning - MoveEvent::loadFunction] Function \""
...
bool MoveEvent::executeStep(Creature* creature, Item* item,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - MoveEvent::executeStep] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool MoveEvent::executeEquip(Player* player, Item* item, slots_t onSlot,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - MoveEvent::executeEquip] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool MoveEvent::executeAddRemItem(Item* item, Item* fromTile,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - MoveEvent::executeAddRemItem] Call stack overflow"
              << std::endl;
    return false;
  }
