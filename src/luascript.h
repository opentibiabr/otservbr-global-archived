...
class ScriptEnvironment {
 public:
  ...
  void getEventInfo(int32_t& scriptId, LuaScriptInterface*& scriptInterface,
...
class LuaScriptInterface {
 public:
  ...
  // Metatables
  static void setMetatable(lua_State* L, int32_t index,
                           const std::string& name);
  static void setWeakMetatable(lua_State* L, int32_t index,
                               const std::string& name);

  static void setItemMetatable(lua_State* L, int32_t index, const Item* item);
  static void setCreatureMetatable(lua_State* L, int32_t index,
                                   const Creature* creature);

  // Get
  template <typename T>
  static typename std::enable_if<std::is_enum<T>::value, T>::type getNumber(
  ...
  static std::string getFieldString(lua_State* L, int32_t arg,
  ...
  static void setField(lua_State* L, const char* index,
 ...
 protected:
  ...
  void registerMethod(const std::string& globalName,
 ...
 private:
  ...
  void registerMetaMethod(const std::string& className,
                          const std::string& methodName, lua_CFunction func);
  void registerGlobalMethod(const std::string& functionName,
                            lua_CFunction func);
  void registerVariable(const std::string& tableName, const std::string& name,
                        lua_Number value);
