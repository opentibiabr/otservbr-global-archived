...
bool Database::rollback() {
  ...
  if (mysql_rollback(handle) != 0) {
    std::cout << "[Error - mysql_rollback] Message: " << mysql_error(handle)
              << std::endl;
    databaseLock.unlock();
    return false;
  }
...
bool Database::commit() {
  ...
  if (mysql_commit(handle) != 0) {
    std::cout << "[Error - mysql_commit] Message: " << mysql_error(handle)
              << std::endl;
    databaseLock.unlock();
    return false;
  }
...
bool Database::executeQuery(const std::string& query) {
  ...
  while (mysql_real_query(handle, query.c_str(), query.length()) != 0) {
    std::cout << "[Error - mysql_real_query] Query: " << query.substr(0, 256)
              << std::endl
...
std::string DBResult::getString(const std::string& s) const {
  ...
  if (it == listNames.end()) {
    std::cout << "[Error - DBResult::getString] Column '" << s
              << "' does not exist in result set." << std::endl;
    return std::string();
  }
