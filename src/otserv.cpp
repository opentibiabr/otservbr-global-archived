...
int main(int argc, char* argv[]) {
  ...
  if (serviceManager.is_running()) {
    ...
    std::cout << ">> No services running. The server is NOT online."
              << std::endl;
...
void mainLoader(int, char* [], ServiceManager* services) {
  ...
  std::cout << "Client Version: " << CLIENT_VERSION_STR << std::endl;
  ...
  const std::string& defaultPriority =
      g_config.getString(ConfigManager::DEFAULT_PRIORITY);
  ...
  if (!DatabaseManager::isDatabaseSetup()) {
    startupErrorMessage(
        "The database you have specified in config.lua is "
  ...
  std::string worldType =
      asLowerCaseString(g_config.getString(ConfigManager::WORLD_TYPE));
  if (worldType == "pvp") {
    ...
    ss << "> ERROR: Unknown world type: "
       << g_config.getString(ConfigManager::WORLD_TYPE)
       << ", valid world types are: pvp, no-pvp"
          " and pvp-enforced.";
  ...
  services->add<ProtocolGame>(
      static_cast<uint16_t>(g_config.getNumber(ConfigManager::GAME_PORT)));
