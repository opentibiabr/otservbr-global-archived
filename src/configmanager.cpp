...
bool ConfigManager::load() {
  ...
  if (luaL_dofile(L, configFileLua.c_str())) {
    std::cout << "[Error - ConfigManager::load] " << lua_tostring(L, -1)
              << std::endl;
    lua_close(L);
    return false;
  }
  ...
  // parse config
  if (!loaded) {  // info that must be loaded one time (unless we reset the
                  // modules involved)
  ...
  boolean[SERVER_SAVE_NOTIFY_MESSAGE] =
      getGlobalBoolean(L, "serverSaveNotifyMessage", true);
  boolean[SERVER_SAVE_CLEAN_MAP] =
      getGlobalBoolean(L, "serverSaveCleanMap", false);
  ...
  boolean[SERVER_SAVE_SHUTDOWN] =
      getGlobalBoolean(L, "serverSaveShutdown", true);
  ...
  integer[FRAG_TIME] =
      getGlobalNumber(L, "timeToDecreaseFrags", 45 * 24 * 60 * 60);
  ...
  integer[SERVER_SAVE_NOTIFY_DURATION] =
      getGlobalNumber(L, "serverSaveNotifyDuration", 5);
...
const std::string& ConfigManager::getString(string_config_t what) const {
  if (what >= LAST_STRING_CONFIG) {
    std::cout
        << "[Warning - ConfigManager::getString] Accessing invalid index: "
        << what << std::endl;
    return dummyStr;
  }
  return string[what];
...
int32_t ConfigManager::getNumber(integer_config_t what) const {
  if (what >= LAST_INTEGER_CONFIG) {
    std::cout
        << "[Warning - ConfigManager::getNumber] Accessing invalid index: "
        << what << std::endl;
    return 0;
  }
  return integer[what];
...
int16_t ConfigManager::getShortNumber(integer_config_t what) const {
  if (what >= LAST_INTEGER_CONFIG) {
    std::cout
        << "[Warning - ConfigManager::getShortNumber] Accessing invalid index: "
        << what << std::endl;
    return 0;
  }
  return integer[what];
...
bool ConfigManager::getBoolean(boolean_config_t what) const {
  if (what >= LAST_BOOLEAN_CONFIG) {
    std::cout
        << "[Warning - ConfigManager::getBoolean] Accessing invalid index: "
        << what << std::endl;
    return false;
  }
  return boolean[what];
...
float ConfigManager::getFloat(floating_config_t what) const {
  if (what >= LAST_FLOATING_CONFIG) {
    std::cout << "[Warning - ConfigManager::getFLoat] Accessing invalid index: "
              << what << std::endl;
    return 0;
  }
