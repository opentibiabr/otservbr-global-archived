...
bool Map::loadMap(const std::string& identifier, bool loadHouses,
  ...
  if (!loader.loadMap(this, identifier)) {
    std::cout << "[Fatal - Map::loadMap] " << loader.getLastErrorString()
              << std::endl;
    return false;
  }
  ...
  if (loadSpawns) {
    if (!IOMap::loadSpawns(this)) {
      std::cout << "[Warning - Map::loadMap] Failed to load spawn data."
                << std::endl;
    }
    std::cout << "> Loaded spawns in: " << (OTSYS_TIME() - start) / (1000.)
              << " seconds" << std::endl;
  }

  if (loadHouses) {
    if (!IOMap::loadHouses(this)) {
      std::cout << "[Warning - Map::loadMap] Failed to load house data."
                << std::endl;
    }
...
Tile* Map::getTile(uint16_t x, uint16_t y, uint8_t z) const {
  ...
  const QTreeLeafNode* leaf =
      QTreeNode::getLeafStatic<const QTreeLeafNode*, const QTreeNode*>(&root, x,
                                                                       y);
  if (!leaf) {
    return nullptr;
  }
...
void Map::setTile(uint16_t x, uint16_t y, uint8_t z, Tile* newTile) {
  if (z >= MAP_MAX_LAYERS) {
    std::cout << "ERROR: Attempt to set tile on invalid coordinate "
...
bool Map::checkSightLine(const Position& fromPos, const Position& toPos) const {
  ...
  const int8_t mx =
      start.x < destination.x ? 1 : start.x == destination.x ? 0 : -1;
  const int8_t my =
      start.y < destination.y ? 1 : start.y == destination.y ? 0 : -1;
...
uint32_t Map::clean() const {
  ...
  std::vector<const QTreeNode*> nodes{&root};
  ...
  std::cout << "> CLEAN: Removed " << count << " item"
            << (count != 1 ? "s" : "") << " from " << tiles << " tile"
            << (tiles != 1 ? "s" : "") << " in "
