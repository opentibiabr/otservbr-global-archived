...
struct ShopInfo {
  ...
  ShopInfo(uint16_t newItemId, int32_t newSubType = 0, uint32_t newBuyPrice = 0,
           uint32_t newSellPrice = 0, std::string newRealName = "")
      : itemId(newItemId),
        subType(newSubType),
        buyPrice(newBuyPrice),
        sellPrice(newSellPrice),
        realName(std::move(newRealName)) {}
};
...
enum MagicEffectsType_t : uint8_t {
  ...
  MAGIC_EFFECTS_DELAY = 2,  // needs uint16_t delay after type to delay in
                            // miliseconds effect display
  MAGIC_EFFECTS_CREATE_EFFECT = 3,  // needs uint8_t effectid after type
  MAGIC_EFFECTS_CREATE_DISTANCEEFFECT =
      4,  // needs uint8_t and deltaX(int8_t), deltaY(int8_t) after type
  MAGIC_EFFECTS_CREATE_DISTANCEEFFECT_REVERSED =
      5,  // needs uint8_t and deltaX(int8_t), deltaY(int8_t) after type
};
