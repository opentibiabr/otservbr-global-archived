...
bool Combat::isProtected(const Player* attacker, const Player* target) {
  uint32_t protectionLevel =
      g_config.getNumber(ConfigManager::PROTECTION_LEVEL);
  ...
  if (attacker) {
    if (const Player* targetPlayer = target->getPlayer()) {
      ...
      if (attacker->isSummon()) {
        if (const Player* masterAttackerPlayer =
    ...
    if (g_game.getWorldType() == WORLD_TYPE_NO_PVP) {
      if (attacker->getPlayer() ||
          (attacker->isSummon() && attacker->getMaster()->getPlayer())) {
        if (target->getPlayer()) {
...
void Combat::CombatFunc(Creature* caster, const Position& pos,
  ...
  g_game.map.getSpectators(spectators, pos, true, true, rangeX, rangeX, rangeY,
                           rangeY);
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - ValueCallback::getMinMaxValues] Call stack overflow"
              << std::endl;
    return;
  }
  ...
  switch (type) {
    ...
    default: {
      std::cout << "ValueCallback::getMinMaxValues - unknown callback type"
                << std::endl;
      scriptInterface->resetScriptEnv();
      return;
    }
...
void TileCallback::onTileCombat(Creature* creature, Tile* tile) const {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - TileCallback::onTileCombat] Call stack overflow"
              << std::endl;
    return;
  }
...
void TargetCallback::onTargetCombat(Creature* creature,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - TargetCallback::onTargetCombat] Call stack overflow"
              << std::endl;
    return;
  }
...
void MagicField::onStepInField(Creature* creature) {
  ...
  if (id == ITEM_MAGICWALL || id == ITEM_WILDGROWTH ||
      id == ITEM_MAGICWALL_SAFE || id == ITEM_WILDGROWTH_SAFE || isBlocking()) {
