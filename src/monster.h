...
class Monster final : public Creature {
 public:
  ...
  bool isPet() const { return mType->info.isPet; }
  bool isPassive() const { return mType->info.isPassive; }
  bool canSee(const Position& pos) const override;
  bool canSeeInvisibility() const override {
    return isImmune(CONDITION_INVISIBLE);
  }
  uint32_t getManaCost() const { return mType->info.manaCost; }
  uint32_t getRespawnType() const { return mType->info.respawnType; }
  void setSpawn(Spawn* newSpawn) { this->spawn = newSpawn; }
