...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	uint32_t version = static_cast<uint32_t>(msg.get<uint32_t>());
	msg.skipBytes(13);
	/*
	 * Skipped bytes:
	...
	if (!Protocol::RSA_decrypt(msg)) {
		spdlog::warn("[ProtocolLogin::onRecvFirstMessage] - RSA Decrypt Failed");
		disconnect();
		return;
	}
	...
	g_dispatcher().addTask(std::bind(&ProtocolLogin::getCharacterList, thisPtr, std::move(accountName), std::move(password), version));
}
