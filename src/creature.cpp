    ...
    if (!summons.empty()) {
      ...
      for (Creature* summon : summons) {
        ...
        if (summon->getMonster()->isPet() &&
            (Position::getDistanceZ(newPos, pos) >= 1 ||
             (std::max<int32_t>(Position::getDistanceX(newPos, pos),
                                Position::getDistanceY(newPos, pos)) >= 15))) {
          g_game.internalTeleport(summon, summon->getMaster()->getPosition(),
                                  true);
        } else if (!summon->getMonster()->isPet() &&
                   (Position::getDistanceZ(newPos, pos) >= 2 ||
                    (std::max<int32_t>(Position::getDistanceX(newPos, pos),
                                       Position::getDistanceY(newPos, pos)) >=
                     30))) {
    ...
    if (isMapLoaded) {
      if (teleport || oldPos.z != newPos.z) {
        ...
        if (oldPos.y > newPos.y) {  // north
          ...
          // update 0
          for (int32_t x = -maxWalkCacheWidth; x <= maxWalkCacheWidth; ++x) {
            Tile* cacheTile = g_game.map.getTile(
                myPos.getX() + x, myPos.getY() - maxWalkCacheHeight, myPos.z);
        ...
        if (oldPos.x < newPos.x) {  // east
          ...
          for (int32_t y = -maxWalkCacheHeight; y <= maxWalkCacheHeight; ++y) {
            Tile* cacheTile = g_game.map.getTile(myPos.x + maxWalkCacheWidth,
                                                 myPos.y + y, myPos.z);
...
bool Creature::dropCorpse(Creature* lastHitCreature,
  ...
  if (!lootDrop && getMonster()) {
    ...
    if (splash) {
      g_game.internalAddItem(updateTile, splash, INDEX_WHEREVER, FLAG_NOLIMIT);
      g_game.startDecay(splash);
    }

    Item* corpse = getCorpse(lastHitCreature, mostDamageCreature);
    if (corpse) {
      g_game.internalAddItem(updateTile, corpse, INDEX_WHEREVER, FLAG_NOLIMIT);
...
bool Creature::addCondition(Condition* condition, bool force /* = false*/) {
  ...
  if (!force && condition->getType() == CONDITION_HASTE &&
      hasCondition(CONDITION_PARALYZE)) {
    int64_t walkDelay = getWalkDelay();
    if (walkDelay > 0) {
      g_scheduler.addEvent(createSchedulerTask(
          walkDelay,
          std::bind(&Game::forceAddCondition, &g_game, getID(), condition)));
      return false;
    }
  }
  ...
  while (it != end) {
    ...
    if (!force && type == CONDITION_PARALYZE) {
      ...
      if (walkDelay > 0) {
        g_scheduler.addEvent(createSchedulerTask(
            walkDelay,
            std::bind(&Game::forceRemoveCondition, &g_game, getID(), type)));
        return;
      }
    }
...
void Creature::removeCondition(Condition* condition, bool force /* = false*/) {
  ...
  if (!force && condition->getType() == CONDITION_PARALYZE) {
    ...
    if (walkDelay > 0) {
      g_scheduler.addEvent(createSchedulerTask(
          walkDelay, std::bind(&Game::forceRemoveCondition, &g_game, getID(),
                               condition->getType())));
      return;
    }
  }
...
bool Creature::getPathTo(const Position& targetPos,
                         ...
                         const FindPathParams& fpp) const {
  return g_game.map.getPathMatching(*this, dirList,
                                    FrozenPathingConditionCall(targetPos), fpp);
}
