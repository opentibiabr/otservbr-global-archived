...
bool Monster::searchTarget(
  ...
  for (Creature* creature : targetList) {
    if (isTarget(creature)) {
      if ((this->mType->info.targetDistance == 1) ||
          canUseAttack(myPos, creature)) {
...
void Monster::onThink(uint32_t interval) {
  ...
  if (mType->info.thinkEvent != -1) {
    ...
    LuaScriptInterface* scriptInterface = mType->info.scriptInterface;
    if (!scriptInterface->reserveScriptEnv()) {
      std::cout << "[Error - Monster::onThink] Call stack overflow"
                << std::endl;
      return;
    }
...
void Monster::onCreatureWalk() { Creature::onCreatureWalk(); }
...
bool Monster::pushItem(Item* item) {
  ...
  for (const auto& it : relList) {
    ...
    if (tile && g_game.canThrowObjectTo(centerPos, tryPos)) {
      if (g_game.internalMoveItem(item->getParent(), tile, INDEX_WHEREVER,
    ...
    switch (playerDir) {
      case DIRECTION_WEST: {
        ...
        if (flee) {
          if (n && s) {
            moveDirection =
                boolean_random() ? DIRECTION_NORTH : DIRECTION_SOUTH;
            return true;
          } else if (n) {
            moveDirection = DIRECTION_NORTH;
            return true;
...
bool Monster::canWalkTo(Position pos, Direction moveDirection) const {
  ...
  if (isInSpawnRange(pos)) {
    ...
    if (walkTile && walkTile->getTopVisibleCreature(this) == nullptr &&
        walkTile->queryAdd(0, *this, 1, FLAG_PATHFINDING) ==
            RETURNVALUE_NOERROR) {
      return true;
    }
  }
