...
Connection_ptr ConnectionManager::createConnection(
    boost::asio::io_service& io_service, ConstServicePort_ptr servicePort) {
...
void Connection::closeSocket() {
  if (socket.is_open()) {
    try {
      ...
      std::cout << "[Network error - Connection::closeSocket] " << e.what()
                << std::endl;
    }
  }
}
...
void Connection::accept() {
  ...
  try {
    ...
    if (!receivedLastChar && receivedName &&
        connectionState == CONNECTION_STATE_CONNECTING_STAGE2) {
      // Read size of the first packet
      boost::asio::async_read(
          socket, boost::asio::buffer(msg.getBuffer(), 1),
    ...
    std::cout << "[Network error - Connection::accept] " << e.what()
              << std::endl;
    close(FORCE_CLOSE);
  }
}
...
void Connection::parseHeader(const boost::system::error_code& error) {
  ...
  if ((++packetsSent / timePassed) >
      static_cast<uint32_t>(
          g_config.getNumber(ConfigManager::MAX_PACKETS_PER_SECOND))) {
  ...
  if (!receivedLastChar &&
    ...
    if (!receivedName && msgBuffer[1] == 0x00) {
      ...
      if (static_cast<char>(msgBuffer[0]) == serverName[serverNameTime]) {
        ...
        std::cout << "[Network error - Connection::parseHeader] "
                  << "Invalid Client Login! Server Name mismatch!" << std::endl;
        close(FORCE_CLOSE);
        return;
      }
    }
  ...
  try {
    ...
    std::cout << "[Network error - Connection::parseHeader] " << e.what()
              << std::endl;
    close(FORCE_CLOSE);
  }
}
...
void Connection::parsePacket(const boost::system::error_code& error) {
  ...
  if (!receivedFirst) {
    ...
    if (!protocol) {
      // As of 11.11+ update, we need to check if it's a outdated client or a
      // status client server with this ugly check
      if (msg.getLength() < 280) {
        msg.skipBytes(
            -NetworkMessage::CHECKSUM_LENGTH);  // those 32bits read up there
      }
  ...
  try {
    ...
    std::cout << "[Network error - Connection::parsePacket] " << e.what()
              << std::endl;
    close(FORCE_CLOSE);
  }
}
...
void Connection::internalSend(const OutputMessage_ptr& conMsg) {
  ...
  try {
    ...
    std::cout << "[Network error - Connection::internalSend] " << e.what()
              << std::endl;
    close(FORCE_CLOSE);
  }
}
