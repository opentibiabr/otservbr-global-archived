  ...
  for (auto eventNode : doc.child("events").children()) {
    ...
    if (res.second) {
      ...
      if (scriptInterface.loadFile("data/events/scripts/" + lowercase +
                                   ".lua") != 0) {
        std::cout << "[Warning - Events::load] Can not load script: "
                  << lowercase << ".lua" << std::endl;
    ...
    if (className == "Creature") {
      if (methodName == "onChangeOutfit") {
        ...
        std::cout << "[Warning - Events::load] Unknown creature method: "
                  << methodName << std::endl;
      }
      ...
      if (methodName == "onJoin") {
        ...
        std::cout << "[Warning - Events::load] Unknown party method: "
                  << methodName << std::endl;
      }
      ...
      if (methodName == "onBrowseField") {
        ...
        std::cout << "[Warning - Events::load] Unknown player method: "
                  << methodName << std::endl;
      }
      ...
      if (methodName == "onDropLoot") {
        ...
        std::cout << "[Warning - Events::load] Unknown monster method: "
                  << methodName << std::endl;
      }
    } else {
      std::cout << "[Warning - Events::load] Unknown class: " << className
                << std::endl;
    }
  }
  return true;
...
void Events::eventMonsterOnSpawn(Monster* monster, const Position& position) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventMonsterOnSpawn] Call stack overflow"
              << std::endl;
    return;
  }
...
bool Events::eventCreatureOnChangeOutfit(Creature* creature,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventCreatureOnChangeOutfit] Call stack overflow"
        << std::endl;
    return false;
  }
...
ReturnValue Events::eventCreatureOnAreaCombat(Creature* creature, Tile* tile,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventCreatureOnAreaCombat] Call stack overflow"
        << std::endl;
...
ReturnValue Events::eventCreatureOnTargetCombat(Creature* creature,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventCreatureOnTargetCombat] Call stack overflow"
        << std::endl;
    return RETURNVALUE_NOTPOSSIBLE;
  }
...
void Events::eventCreatureOnHear(Creature* creature, Creature* speaker,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventCreatureOnHear] Call stack overflow"
              << std::endl;
    return;
  }
...
void Events::eventCreatureOnDrainHealth(Creature* creature, Creature* attacker,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventCreatureOnDrainHealth] Call stack overflow"
        << std::endl;
    return;
  }
...
bool Events::eventPartyOnJoin(Party* party, Player* player) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPartyOnJoin] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Events::eventPartyOnLeave(Party* party, Player* player) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPartyOnLeave] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Events::eventPartyOnDisband(Party* party) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPartyOnDisband] Call stack overflow"
              << std::endl;
    return false;
  }
...
void Events::eventPartyOnShareExperience(Party* party, uint64_t& exp) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPartyOnShareExperience] Call stack overflow"
        << std::endl;
    return;
  }
...
bool Events::eventPlayerOnBrowseField(Player* player,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnBrowseField] Call stack overflow"
        << std::endl;
    return false;
  }
...
void Events::eventPlayerOnLook(Player* player, const Position& position,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnLook] Call stack overflow"
              << std::endl;
    return;
  }
...
void Events::eventPlayerOnLookInBattleList(Player* player, Creature* creature,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnLookInBattleList] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerOnLookInTrade(Player* player, Player* partner,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnLookInTrade] Call stack overflow"
        << std::endl;
    return;
  }
...
bool Events::eventPlayerOnLookInShop(Player* player, const ItemType* itemType,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnLookInShop] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Events::eventPlayerOnMoveItem(Player* player, Item* item, uint16_t count,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnMoveItem] Call stack overflow"
              << std::endl;
    return false;
  }
...
void Events::eventPlayerOnItemMoved(Player* player, Item* item, uint16_t count,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnItemMoved] Call stack overflow"
              << std::endl;
    return;
  }
...
bool Events::eventPlayerOnMoveCreature(Player* player, Creature* creature,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnMoveCreature] Call stack overflow"
        << std::endl;
    return false;
  }
...
void Events::eventPlayerOnReportRuleViolation(Player* player,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnReportRuleViolation] Call "
                 "stack overflow"
              << std::endl;
    return;
  }
...
bool Events::eventPlayerOnReportBug(Player* player, const std::string& message,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnReportBug] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Events::eventPlayerOnTurn(Player* player, Direction direction) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnTurn] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Events::eventPlayerOnTradeRequest(Player* player, Player* target,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnTradeRequest] Call stack overflow"
        << std::endl;
    return false;
  }
...
bool Events::eventPlayerOnTradeAccept(Player* player, Player* target,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnTradeAccept] Call stack overflow"
        << std::endl;
    return false;
  }
...
void Events::eventPlayerOnGainExperience(Player* player, Creature* source,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnGainExperience] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerOnLoseExperience(Player* player, uint64_t& exp) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnLoseExperience] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerOnGainSkillTries(Player* player, skills_t skill,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnGainSkillTries] Call stack overflow"
        << std::endl;
    return;
  }
...
bool Events::eventPlayerCanBeAppliedImbuement(Player* player,
                                              Imbuement* imbuement,
                                              Item* item) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::canBeAppliedImbuemet] Call stack overflow"
              << std::endl;
    return false;
  }
...
void Events::eventPlayerOnApplyImbuement(Player* player, Imbuement* imbuement,
                                         Item* item, uint8_t slot,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnApplyImbuement] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerClearImbuement(Player* player, Item* item,
                                       uint8_t slot) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerClearImbuement] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerOnCombat(Player* player, Creature* target, Item* item,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventPlayerOnCombat] Call stack overflow"
              << std::endl;
    return;
  }
...
void Events::eventPlayerOnRequestQuestLog(Player* player) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnRequestQuestLog] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventPlayerOnRequestQuestLine(Player* player, uint16_t questId) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout
        << "[Error - Events::eventPlayerOnRequestQuestLine] Call stack overflow"
        << std::endl;
    return;
  }
...
void Events::eventOnStorageUpdate(Player* player, const uint32_t key,
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventOnStorageUpdate] Call stack overflow"
              << std::endl;
    return;
  }
...
void Events::eventMonsterOnDropLoot(Monster* monster, Container* corpse) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    std::cout << "[Error - Events::eventMonsterOnDropLoot] Call stack overflow"
              << std::endl;
    return;
  }
