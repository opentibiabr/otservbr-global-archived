...
class Game {
 public:
  ...
  /**
   * Returns a player based on a string name identifier, with support for the
   * "~" wildcard. \param s is the name identifier, with or without wildcard
   * \param player will point to the found player (if any)
   * \return "RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE" or
   ...
   * tiles away \param forced If true, placing the creature will not fail
   * because of obstacles (creatures/items)
   */
   ...
   * \param pos The position to place the creature
   * \param extendedPos If true, the creature will in first-hand be placed 2
   * tiles away \param force If true, placing the creature will not fail because
   * of obstacles (creatures/items)
   */
   ...
   * charges/fluidtype, default is -1 meaning it's not used \param depthSearch
   * if true it will check child containers aswell \returns A pointer to the
   * item to an item and nullptr if not found
   */
   ...
   * \param newCount is the new count value, use default value (-1) to not
   * change it \returns true if the tranformation was successful
   */
   ...
   */
  ReturnValue internalQuickLootItem(
  ...
  void playerStowItem(Player* player, const Position& pos, uint16_t spriteId,
                      uint8_t stackpos, uint32_t count);
  void playerStowContainer(Player* player, const Position& pos,
                           uint16_t spriteId, uint8_t stackpos);
  void playerStowAllItems(Player* player, const Position& pos,
                          uint16_t spriteId, uint8_t stackpos);
  void playerStashWithdraw(Player* player, uint16_t spriteId, uint32_t count,
                           uint8_t stackpos);
  ...
  void playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer,
                         uint8_t index);
  void playerPurchaseItem(uint32_t playerId, uint16_t spriteId, uint8_t count,
  ...
  void playerRequestEditVip(uint32_t playerId, uint32_t guid,
                            ...
                            bool notify);
  void playerApplyImbuement(uint32_t playerId, uint32_t imbuementid,
  ...
  void playerCreateMarketOffer(uint32_t playerId, uint8_t type,
                               uint16_t spriteId, uint16_t amount,
  ...
  std::forward_list<Item*> getMarketItemList(uint16_t wareId,
