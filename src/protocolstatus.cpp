  ...
  if (ip != 0x0100007F) {
    ...
    if (ipStr != g_config.getString(ConfigManager::IP)) {
      ...
      if (it != ipConnectMap.end() &&
          (OTSYS_TIME() <
           (it->second +
            g_config.getNumber(ConfigManager::STATUSQUERY_TIMEOUT)))) {
        disconnect();
        return;
      }
    }
...
void ProtocolStatus::sendStatusString() {
  ...
  serverinfo.append_attribute("ip") =
      g_config.getString(ConfigManager::IP).c_str();
  ...
  serverinfo.append_attribute("location") =
      g_config.getString(ConfigManager::LOCATION).c_str();
  serverinfo.append_attribute("url") =
      g_config.getString(ConfigManager::URL).c_str();
  ...
  owner.append_attribute("name") =
      g_config.getString(ConfigManager::OWNER_NAME).c_str();
  owner.append_attribute("email") =
      g_config.getString(ConfigManager::OWNER_EMAIL).c_str();
  ...
  rates.append_attribute("experience") =
      std::to_string(g_config.getNumber(ConfigManager::RATE_EXPERIENCE))
          .c_str();
  ...
  map.append_attribute("name") =
      g_config.getString(ConfigManager::MAP_NAME).c_str();
  map.append_attribute("author") =
      g_config.getString(ConfigManager::MAP_AUTHOR).c_str();
...
void ProtocolStatus::sendInfo(uint16_t requestedInfo,
  ...
  if (requestedInfo & REQUEST_BASIC_SERVER_INFO) {
    ...
    output->addString(
        std::to_string(g_config.getNumber(ConfigManager::LOGIN_PORT)));
  }
