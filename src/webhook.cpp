...
void webhook_init() {
	if (curl_global_init(CURL_GLOBAL_ALL) != 0) {
		spdlog::error("Failed to init curl, no webhook messages may be sent");
		return;
	}
	...
	if (headers == NULL) {
		spdlog::error("Failed to init curl, appending request headers failed");
		return;
	}
...
void webhook_send_message(std::string title, std::string message, int color) {
	...
	if (!init) {
		spdlog::error("Failed to send webhook message; Did not (successfully) init");
		return;
	}

	if (title.empty() || message.empty()) {
		spdlog::error("Failed to send webhook message; "
...
static int webhook_send_message_(const char *url, const char *payload, std::string *response_body) {
	...
	if (!curl) {
		spdlog::error("Failed to send webhook message; curl_easy_init failed");
		return -1;
	}
	...
	if (res == CURLE_OK) {
		...
		spdlog::error("Failed to send webhook message with the error: {}",
