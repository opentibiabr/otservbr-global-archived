...
void Player::addStorageValue(const uint32_t key, const int32_t value, const bool isLogin/* = false*/)
{
	if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
		if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
			...
			spdlog::warn("Unknown reserved key: {} for player: {}", key, getName());
			return;
		}
	}
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		spdlog::info("{} has logged out", getName());
		...
		if (!saved) {
			spdlog::warn("Error while saving player: {}", getName());
		}
	}
}
...
void Player::setGuild(Guild* newGuild)
{
	...
	if (newGuild) {
		const GuildRank* rank = newGuild->getRankByLevel(1);
		if (!rank) {
			return;
		}
