...
Reward* Player::getReward(uint32_t rewardId, bool autoCreate) {
  ...
  g_game.internalAddItem(getRewardChest(), reward, INDEX_WHEREVER,
                         FLAG_NOLIMIT);
      ...
      if (slotPosition & SLOTP_RIGHT) {
        if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
          if (item->getWeaponType() != WEAPON_SHIELD) {
            ...
            if (leftItem) {
              if ((leftItem->getSlotPosition() | slotPosition) &
                  SLOTP_TWO_HAND) {
    ...
    for (int32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_LAST;
      ...
      if (inventoryItem) {
        if (Container* subContainer = inventoryItem->getContainer()) {
          ...
          subContainer->queryMaxCount(INDEX_WHEREVER, *item,
          ...
          for (ContainerIterator it = subContainer->iterator(); it.hasNext();
            ...
            if (Container* tmpContainer = (*it)->getContainer()) {
              queryCount = 0;
              tmpContainer->queryMaxCount(INDEX_WHEREVER, *item,
...
Cylinder* Player::queryDestination(int32_t& index, const Thing& thing,
  ...
  if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREVER) {
    *destItem = nullptr;
...
void Player::stowContainer(Item* item, uint32_t count,
  ...
  for (Item* i : itemList) {
    ...
    itemDict[i->getClientID()] =
        std::pair<bool, uint32_t>(false, sameItemCountSum);
  }
  ...
  itemDict = IOStash::stashContainer(
      this->guid, itemDict, g_config.getNumber(ConfigManager::STASH_ITEMS));
  ...
  if (itemDict.size() == 0) {
    if (itemList.size() == 0)
      ...
      sendCancelMessage(
          "You don't have capacity in the Supply Stash to store this item");
    else
      sendCancelMessage(
          "You don't have capacity in the Supply Stash to store this "
          "container");
    return;
  }
...
bool Player::isItemStorable(Item* item) {
  auto isContainerAndHasSomethingInside =
      ...
      item->getContainer()->getItemList().size() > 0;
