...
bool House::transferToDepot(Player* player) const {
  ...
  int constructionKits[58][2] = {
      ...
      {11126, 11127}, {11133, 11129}, {11205, 11203}, {14328, 1616},
      {14329, 1615},  {16075, 16020}, {16099, 16098}, {20254, 20295},
      {20255, 20297}, {20257, 20299},
  };
        ...
        if (item->isWrapable()) {
          ...
          for (int i = 0; i < 58; i++) {
            if (constructionKits[i][1] == item->getID()) {
              Item* newItem =
                  g_game.transformItem(item, constructionKits[i][0]);
              moveItemList.push_back(newItem);
              break;
            }
            if (item->isRotatable()) {
  ...
  for (Item* item : moveItemList) {
    g_game.internalMoveItem(item->getParent(), player->getInbox(),
                            INDEX_WHEREVER, item, item->getItemCount(),
...
bool Houses::loadHousesXML(const std::string& filename) {
  ...
  for (auto houseNode : doc.child("houses").children()) {
    ...
    if (!house) {
      std::cout << "Error: [Houses::loadHousesXML] Unknown house, id = "
                << houseId << std::endl;
      return false;
    }
    ...
    if (entryPos.x == 0 && entryPos.y == 0 && entryPos.z == 0) {
      std::cout << "[Warning - Houses::loadHousesXML] House entry not set"
                << " - Name: " << house->getName() << " - House id: " << houseId
                << std::endl;
    }
  ...
  for (const auto& it : houseMap) {
    ...
    if (player.getBankBalance() >= rent) {
      ...
      if (house->getPayRentWarnings() < 7) {
        ...
        g_game.internalAddItem(player.getInbox(), letter, INDEX_WHEREVER,
                               FLAG_NOLIMIT);
