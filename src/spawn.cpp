  ...
  for (auto spawnNode : doc.child("spawns").children()) {
    ...
    for (auto childNode : spawnNode.children()) {
      if (strcasecmp(childNode.name(), "monster") == 0) {
        ...
        if (!BoostedName) {
          std::cout << "[Warning - Boosted creature] Failed to detect boosted "
                       "creature database."
                    << std::endl;
          return false;
        }
        ...
        uint32_t interval =
            pugi::cast<uint32_t>(childNode.attribute("spawntime").value()) *
            1000 /
            (g_config.getNumber(ConfigManager::RATE_SPAWN) * boostedrate);
        if (interval > MINSPAWN_INTERVAL) {
          spawn.addMonster(nameAttribute.as_string(), pos, dir, interval);
        } else {
          std::cout << "[Warning - Spawns::loadFromXml] "
                    << nameAttribute.as_string() << ' ' << pos
                    << " spawntime can not be less than "
        ...
        pugi::xml_attribute directionAttribute =
            childNode.attribute("direction");
        if (directionAttribute) {
          npc->setDirection(static_cast<Direction>(
              pugi::cast<uint16_t>(directionAttribute.value())));
        }

        npc->setMasterPos(
            Position(centerPos.x +
                         pugi::cast<uint16_t>(childNode.attribute("x").value()),
                     centerPos.y +
                         pugi::cast<uint16_t>(childNode.attribute("y").value()),
                     centerPos.z),
...
bool Spawns::loadCustomSpawnXml(const std::string& _filename) {
  if (!loaded) {
    std::cout << "Error - Spawns::loadCustomSpawnXml - trying to load custom "
                 "spawn xml before game startup. FileName: "
              << _filename << std::endl;
    return false;
  }
  ...
  for (pugi::xml_node spawnNode = doc.child("spawns").first_child(); spawnNode;
    ...
    for (pugi::xml_node childNode = spawnNode.first_child(); childNode;
      ...
      if (strcasecmp(childNode.name(), "monster") == 0) {
        ...
        if (interval > MINSPAWN_INTERVAL) {
          ...
          std::cout << "[Warning - Spawns::loadCustomSpawnXml - '"
                    << _filename.c_str() << "'] " << nameAttribute.as_string()
                    << ' ' << pos << " spawntime can not be less than "
                    << MINSPAWN_INTERVAL / 1000 << " seconds." << std::endl;
        }
      } else if (strcasecmp(childNode.name(), "npc") == 0) {
        pugi::xml_attribute nameAttribute = childNode.attribute("name");
...
bool Spawns::isInZone(const Position& centerPos, int32_t radius,
                      const Position& pos) {
  if (radius == -1) {
    return true;
  }
...
void Spawn::startSpawnCheck() {
  if (checkSpawnEvent == 0) {
    checkSpawnEvent = g_scheduler.addEvent(createSchedulerTask(
        getInterval(), std::bind(&Spawn::checkSpawn, this)));
  }
}
...
bool Spawn::spawnMonster(uint32_t spawnId, MonsterType* mType,
                         const Position& pos, Direction dir,
                         bool startup /*= false*/) {
...
void Spawn::checkSpawn() {
  ...
  for (auto& it : spawnMap) {
    ...
    if (OTSYS_TIME() >= sb.lastSpawn + sb.interval) {
      ...
      if (++spawnCount >= static_cast<uint32_t>(
...
void Spawn::scheduleSpawn(uint32_t spawnId, spawnBlock_t& sb,
                          uint16_t interval) {
  if (interval <= 0) {
    spawnMonster(spawnId, sb.mType, sb.pos, sb.direction);
  } else {
    ...
    g_scheduler.addEvent(createSchedulerTask(
        1400, std::bind(&Spawn::scheduleSpawn, this, spawnId, sb,
                        interval - NONBLOCKABLE_SPAWN_INTERVAL)));
  }
}
...
bool Spawn::addMonster(const std::string& name, const Position& pos,
