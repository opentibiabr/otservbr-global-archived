...
StashItemList IOStash::getStoredItems(uint32_t playerId) {
  ...
  query << "SELECT `item_id`, `item_count` FROM `player_stash` WHERE "
           "`player_id` = "
        << playerId;
...
uint16_t IOStash::getStashItemCount(uint32_t playerId, uint32_t spriteId) {
  std::ostringstream query;
  query << "SELECT `item_count` FROM `player_stash` WHERE `player_id` = "
        << playerId << " AND `item_id` = " << spriteId;
...
std::map<uint16_t, std::pair<bool, uint32_t>> IOStash::stashContainer(
  ...
  insertQuery << "INSERT INTO `player_stash` "
                 "(`player_id`,`item_id`,`item_count`) VALUES ";
...
bool IOStash::withdrawItem(uint32_t playerId, uint16_t itemId, uint16_t count) {
  ...
  if (result) {
    ...
    std::cout << "[IOStash::withdrawItem] could not withdraw itemId " << itemId
              << " from playerId " << playerId << std::endl;
    return false;
  }

  if (remainingCount != 0) {
    query << "UPDATE `player_stash` SET `item_count` = " << remainingCount
