...
void IOMapSerialize::loadHouseItems(Map* map) {
  ...
  std::cout << "> Loaded house items in: " << (OTSYS_TIME() - start) / (1000.)
            << " seconds" << std::endl;
}
...
bool IOMapSerialize::saveHouseItems() {
  ...
  std::cout << "> Saved house items in: " << (OTSYS_TIME() - start) / (1000.)
            << " seconds" << std::endl;
  return success;
}
...
bool IOMapSerialize::loadContainer(PropStream& propStream,
  ...
  while (container->serializationCount > 0) {
    if (!loadItem(propStream, container)) {
      std::cout << "[Warning - IOMapSerialize::loadContainer] Unserialization "
                   "error for container item: "
  ...
  if (iType.moveable || !tile || iType.isCarpet()) {
    ...
    if (item) {
      if (item->unserializeAttr(propStream)) {
        ...
        std::cout
            << "WARNING: Unserialization error in IOMapSerialize::loadItem()"
            << id << std::endl;
        delete item;
        return false;
      }
