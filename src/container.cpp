...
std::ostringstream& Container::getContentDescription(
...
ReturnValue Container::queryAdd(int32_t addIndex, const Thing& addThing,
  ...
  if (!hasBitSet(FLAG_NOLIMIT, flags)) {
    ...
    if (addIndex == INDEX_WHEREVER && size() >= capacity() &&
        !hasPagination()) {
      return RETURNVALUE_CONTAINERNOTENOUGHROOM;
    }
  } else {
  ...
  if (const Container* topParentContainer = getTopParentContainer()) {
    uint32_t maxItem =
        static_cast<uint32_t>(g_config.getNumber(ConfigManager::MAX_ITEM));
    if (const Container* addContainer = item->getContainer()) {
      uint32_t addContainerCount = addContainer->getContainerHoldingCount() + 1;
      uint32_t maxContainer = static_cast<uint32_t>(
          g_config.getNumber(ConfigManager::MAX_CONTAINER));
  ...
  if (topParent != this) {
    return topParent->queryAdd(INDEX_WHEREVER, *item, addCount,
