...
bool GlobalEvents::registerEvent(Event_ptr event, const pugi::xml_node&) {
  ...
  std::cout << "[Warning - GlobalEvents::configureEvent] Duplicate registered "
               "globalevent with name: "
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node) {
  ...
  if (!nameAttribute) {
    std::cout << "[Error - GlobalEvent::configureEvent] Missing name for a "
                 "globalevent"
              << std::endl;
    return false;
  }
  ...
  if ((attr = node.attribute("time"))) {
    ...
    if (hour < 0 || hour > 23) {
      std::cout << "[Error - GlobalEvent::configureEvent] Invalid hour \""
                << attr.as_string() << "\" for globalevent with name: " << name
                << std::endl;
      return false;
    }
    ...
    if (strcasecmp(value, "startup") == 0) {
      ...
      std::cout << "[Error - GlobalEvent::configureEvent] No valid type \""
                << attr.as_string() << "\" for globalevent with name " << name
                << std::endl;
      return false;
    }
    ...
    std::cout << "[Error - GlobalEvent::configureEvent] No interval for "
                 "globalevent with name "
              << name << std::endl;
    return false;
  }
...
bool GlobalEvent::executeRecord(uint32_t current, uint32_t old) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - GlobalEvent::executeRecord] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool GlobalEvent::executeEvent() const {
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - GlobalEvent::executeEvent] Call stack overflow"
              << std::endl;
    return false;
  }
