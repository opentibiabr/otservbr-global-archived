...
bool IOMap::loadMap(Map* map, const std::string& fileName) {
  ...
  if (headerVersion <= 0) {
    ...
    // during an update.
    setLastErrorString(
        "This map need to be upgraded by using the latest map editor version "
  ...
  if (root_header.majorVersionItems > Item::items.majorVersion) {
    setLastErrorString(
        "The map was saved with a different items.otb version, an upgraded "
        "items.otb is required.");
    return false;
  }
  ...
  std::cout << "> Map size: " << root_header.width << "x" << root_header.height
            << std::endl;
  ...
  std::cout << "> Map loading time: " << (OTSYS_TIME() - start) / (1000.)
            << " seconds" << std::endl;
  return true;
}
        ...
        case OTBM_ATTR_ITEM: {
          ...
          if (Teleport* teleport = item->getTeleport()) {
            ...
            if (it != teleportMap.end()) {
              std::cout << "[Warning - IOMap::loadMap] Teleport in position [x:"
                        << x << ", y : " << y << ", z : " << z
                        << "] is leading to another teleport." << std::endl;
            }
            for (auto const& it2 : teleportMap) {
              if (it2.second == teleportPosition) {
                ...
                std::cout
                    << "[Warning - IOMap::loadMap] Teleport in position [x:"
                    << fx << ", y : " << fy
                    << ", z : " << static_cast<uint16_t>(fz)
                    << "] is leading to another teleport." << std::endl;
              }
            }
          }
          ...
          if (isHouseTile && item->isMoveable()) {
            std::cout << "[Warning - IOMap::loadMap] Moveable item with ID: "
                      << item->getID() << ", in house: " << house->getId()
                      << ", at position [x: " << x << ", y: " << y
                      << ", z: " << z << "]." << std::endl;
            delete item;
          } else {
