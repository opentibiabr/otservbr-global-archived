...
bool ChatChannel::executeCanJoinEvent(const Player& player) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - CanJoinChannelEvent::execute] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool ChatChannel::executeOnJoinEvent(const Player& player) {
  ...
  LuaScriptInterface* scriptInterface = g_chat->getScriptInterface();
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - OnJoinChannelEvent::execute] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool ChatChannel::executeOnLeaveEvent(const Player& player) {
  ...
  LuaScriptInterface* scriptInterface = g_chat->getScriptInterface();
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - OnLeaveChannelEvent::execute] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool ChatChannel::executeOnSpeakEvent(const Player& player, SpeakClasses& type,
  ...
  LuaScriptInterface* scriptInterface = g_chat->getScriptInterface();
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - OnSpeakChannelEvent::execute] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Chat::load() {
  ...
  if (!result) {
    printXMLError("Error - Chat::load", "data/chatchannels/chatchannels.xml",
                  result);
    return false;
  }
    ...
    if (it != normalChannels.end()) {
      ...
      if (scriptAttribute) {
        if (scriptInterface.loadFile(
          ...
          std::cout << "[Warning - Chat::load] Can not load script: "
                    << scriptAttribute.as_string() << std::endl;
        }
      }
