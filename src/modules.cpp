...
bool Modules::registerEvent(Event_ptr event, const pugi::xml_node&) {
  ...
  if (module->getEventType() == MODULE_TYPE_NONE) {
    std::cout << "Error: [Modules::registerEvent] Trying to register event "
                 "without type!"
              << std::endl;
    return false;
  }
...
void Modules::executeOnRecvbyte(Player* player, NetworkMessage& msg,
                                uint8_t byte) const {
  for (auto& it : recvbyteList) {
...
bool Module::configureEvent(const pugi::xml_node& node) {
  ...
  if (!typeAttribute) {
    std::cout << "[Error - Modules::configureEvent] Missing type for module."
              << std::endl;
    return false;
  }
  ...
  if (tmpStr == "recvbyte") {
    ...
    if (!byteAttribute) {
      std::cout << "[Error - Modules::configureEvent] Missing byte for module "
                   "typed recvbyte."
                << std::endl;
      return false;
    }
    ...
    std::cout << "[Error - Modules::configureEvent] Invalid type for module."
              << std::endl;
    return false;
  }
