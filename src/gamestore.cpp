    ...
    for (auto categoryNode :
      ...
      for (auto offerNode : categoryNode.children()) {
        ...
        if (boost::iequals(type, "namechange")) {
          ...
          if (!tmp->mountId) {
            printXMLError(
                "Error parsing XML mountID number not specified for an mount "
                "offer - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          } else {
            offer = tmp;
          ...
          if (!tmp->productId || !tmp->count) {
            printXMLError(
                "Error parsing XML Stackable Item Offer - "
          ...
          if (!tmp->productId || !tmp->count) {
            printXMLError(
                "Error parsing XML Wrappable Item Offer - "
                "GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          } else {
          ...
          if (!tmp->blessings.size()) {
            // no number was found
            printXMLError(
                "Error Parsing XML bless offer - no blessnumber specified  - "
                "GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          }
          ...
          if (tmp->days == 0) {
            printXMLError(
                "Error parsing XML premiumtime offer type - required 'days' "
                "attribute not found - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          }
        ...
        if (!offer) {
          ...
          if (!offer->name.length() || !offer->price) {
            printXMLError(
                "Error parsing XML - One or more required offer params are "
                "missing - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          }
          offerCount++;
