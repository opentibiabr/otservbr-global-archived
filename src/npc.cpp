...
bool Npc::loadFromXml() {
  ...
  if (!npcNode) {
    std::cout << "[Error - Npc::loadFromXml] Missing npc tag in " << filename
              << std::endl;
    return false;
  }
  ...
  if (lookNode) {
    ...
    if (lookTypeAttribute) {
      ...
      defaultOutfit.lookHead =
          pugi::cast<uint16_t>(lookNode.attribute("head").value());
      defaultOutfit.lookBody =
          pugi::cast<uint16_t>(lookNode.attribute("body").value());
      defaultOutfit.lookLegs =
          pugi::cast<uint16_t>(lookNode.attribute("legs").value());
      defaultOutfit.lookFeet =
          pugi::cast<uint16_t>(lookNode.attribute("feet").value());
      defaultOutfit.lookAddons =
          pugi::cast<uint16_t>(lookNode.attribute("addons").value());
    } else if ((attr = lookNode.attribute("typeex"))) {
      defaultOutfit.lookTypeEx = pugi::cast<uint16_t>(attr.value());
    }
    defaultOutfit.lookMount =
        pugi::cast<uint16_t>(lookNode.attribute("mount").value());
...
void Npc::onPlayerTrade(Player* player, int32_t callback, uint16_t itemId,
                        uint8_t count, uint8_t amount, bool ignore /* = false*/,
  ...
  g_dispatcher.addTask(createTask(
      std::bind(&Game::updatePlayerSaleItems, &g_game, player->getID())));
  player->setScheduledSaleUpdate(true);
  if (npcEventHandler) {
    npcEventHandler->onPlayerTrade(player, callback, itemId, count, amount,
                                   ignore, inBackpacks);
  }
}
...
bool NpcScriptInterface::loadNpcLib(const std::string& file) {
  ...
  if (loadFile(file) == -1) {
    std::cout << "[Warning - NpcScriptInterface::loadNpcLib] Can not load "
              << file << std::endl;
    return false;
  }
...
void NpcScriptInterface::registerFunctions() {
  ...
  lua_register(luaState, "doNpcSetCreatureFocus",
               NpcScriptInterface::luaSetNpcFocus);
  lua_register(luaState, "getNpcCid", NpcScriptInterface::luaGetNpcCid);
  lua_register(luaState, "getNpcParameter",
               NpcScriptInterface::luaGetNpcParameter);
  lua_register(luaState, "openShopWindow",
               NpcScriptInterface::luaOpenShopWindow);
  lua_register(luaState, "closeShopWindow",
               NpcScriptInterface::luaCloseShopWindow);
  ...
  registerMethod("Npc", "openShopWindow",
                 NpcScriptInterface::luaNpcOpenShopWindow);
  registerMethod("Npc", "closeShopWindow",
                 NpcScriptInterface::luaNpcCloseShopWindow);
}
...
int NpcScriptInterface::luaActionMoveTo(lua_State* L) {
  ...
  npc->doMoveTo(Position(getNumber<uint16_t>(L, 1), getNumber<uint16_t>(L, 2),
...
int NpcScriptInterface::luaOpenShopWindow(lua_State* L) {
  ...
  while (lua_next(L, -2) != 0) {
    ...
    uint16_t itemId =
        static_cast<uint16_t>(getField<uint32_t>(L, tableIndex, "id"));
    ...
    items.emplace_back(itemId, subType, buyPrice, sellPrice,
                       std::move(realName));
    lua_pop(L, 6);
  }
  lua_pop(L, 1);
...
int NpcScriptInterface::luaDoSellItem(lua_State* L) {
  // doSellItem(cid, itemid, amount, <optional> subtype, <optional> actionid,
  // <optional: default: 1> canDropOnMap)
  Player* player = getPlayer(L, 1);
  if (!player) {
    reportErrorFunc(getErrorDesc(LUA_ERROR_PLAYER_NOT_FOUND));
    pushBoolean(L, false);
...
NpcEventsHandler::NpcEventsHandler(const std::string& file, Npc* npcEvent)
  ...
  if (!loaded) {
    std::cout << "[Warning - NpcScript::NpcScript] Can not load script: "
              << file << std::endl;
...
void NpcEventsHandler::onCreatureAppear(Creature* creature) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onCreatureAppear] Call stack overflow"
              << std::endl;
  }
...
void NpcEventsHandler::onCreatureDisappear(Creature* creature) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onCreatureDisappear] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onCreatureMove(Creature* creature,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onCreatureMove] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onCreatureSay(Creature* creature, SpeakClasses type,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onCreatureSay] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onPlayerTrade(Player* player, int32_t callback,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onPlayerTrade] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onPlayerCloseChannel(Player* player) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onPlayerCloseChannel] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onPlayerEndTrade(Player* player) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onPlayerEndTrade] Call stack overflow"
              << std::endl;
    return;
  }
...
void NpcEventsHandler::onThink() {
  ...
  // onThink()
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - NpcScript::onThink] Call stack overflow"
              << std::endl;
    return;
  }
