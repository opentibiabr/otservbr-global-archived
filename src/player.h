...
class Player final : public Creature, public Cylinder {
 public:
  ...
  // New Prey
  uint16_t getPreyState(uint16_t slot) const { return preySlotState[slot]; }
  ...
  uint32_t getPreyNextUse(uint16_t slot) const { return preySlotNextUse[slot]; }
  ...
  uint16_t getPreyBonusRerolls() const { return preyBonusRerolls; }

  uint16_t getPreyTick(uint16_t slot) const { return preySlotTick[slot]; }
  //

  uint16_t getPreyStamina(uint16_t index) const {
    return preyStaminaMinutes[index];
  }
  uint16_t getPreyType(uint16_t index) const { return preyBonusType[index]; }
  uint16_t getPreyValue(uint16_t index) const { return preyBonusValue[index]; }
  std::string getPreyName(uint16_t index) const { return preyBonusName[index]; }
  ...
  bool inImbuing() { return imbuing != nullptr; }
  ...
  void setSupplyStashAvailable(bool value) { supplyStashAvailable = value; }
  ...
  uint16_t getSkillLevel(uint8_t skill) const {
    if (skill == SKILL_LIFE_LEECH_CHANCE || skill == SKILL_MANA_LEECH_CHANCE) {
      return std::min<uint16_t>(
          100, std::max<uint16_t>(0, skills[skill].level + varSkills[skill]));
    }
  ...
  void sendSaleItemList(
      const std::map<uint32_t, uint32_t>& inventoryMap) const {
    if (client) {
  ...
  void sendMarketBrowseOwnHistory(
 ...
 private:
  ...
  void getAllItemTypeCountAndSubtype(
  ...
  std::forward_list<Condition*>
      storedConditionList;  // TODO: This variable is only temporarily used when
                            // logging in, get rid of it somehow
