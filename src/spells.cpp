...
bool Spells::registerEvent(Event_ptr event, const pugi::xml_node&) {
  ...
  if (instant) {
    ...
    if (!result.second) {
      std::cout << "[Warning - Spells::registerEvent] Duplicate registered "
                   "instant spell with words: "
...
bool Spells::registerInstantLuaEvent(InstantSpell* event) {
  ...
  if (instant) {
    ...
    auto result = instants.emplace(instant->getWords(), std::move(*instant));
    if (!result.second) {
      std::cout << "[Warning - Spells::registerInstantLuaEvent] Duplicate "
                   "registered instant spell with words: "
                << words << std::endl;
    }
    return result.second;
  }
...
bool Spells::registerRuneLuaEvent(RuneSpell* event) {
  ...
  if (rune) {
    ...
    auto result = runes.emplace(rune->getRuneItemId(), std::move(*rune));
    if (!result.second) {
      std::cout << "[Warning - Spells::registerRuneLuaEvent] Duplicate "
                   "registered rune with id: "
                << id << std::endl;
    }
    return result.second;
  }
...
bool CombatSpell::executeCastSpell(Creature* creature, const LuaVariant& var) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - CombatSpell::executeCastSpell] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool Spell::configureSpell(const pugi::xml_node& node) {
  ...
  if (!nameAttribute) {
    std::cout << "[Error - Spell::configureSpell] Spell without name"
              << std::endl;
    return false;
  }
  ...
  for (const char* reserved : reservedList) {
    if (strcasecmp(reserved, name.c_str()) == 0) {
      std::cout
          << "[Error - Spell::configureSpell] Spell is using a reserved name: "
          << reserved << std::endl;
      return false;
    }
  }
  ...
  if ((attr = node.attribute("group"))) {
    ...
    if (tmpStr == "none" || tmpStr == "0") {
      ...
      std::cout << "[Warning - Spell::configureSpell] Unknown group: "
  ...
  if ((attr = node.attribute("secondarygroup"))) {
    ...
    if (tmpStr == "none" || tmpStr == "0") {
      ...
      std::cout << "[Warning - Spell::configureSpell] Unknown secondarygroup: "
                << attr.as_string() << std::endl;
    }
  }
  ...
  if ((attr = node.attribute("blocktype"))) {
    ...
    if (tmpStrValue == "all") {
      ...
      std::cout << "[Warning - Spell::configureSpell] Blocktype \""
  ...
  for (auto vocationNode : node.children()) {
    ...
    if (vocationId != -1) {
      ...
      std::cout << "[Warning - Spell::configureSpell] Wrong vocation name: "
                << attr.as_string() << std::endl;
    }
  }
  return true;
    ...
    if (!player->hasFlag(PlayerFlag_HasNoExhaustion)) {
      if (cooldown > 0) {
        Condition* condition = Condition::createCondition(
            CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, cooldown, 0, false,
            spellId);
      ...
      if (groupCooldown > 0) {
        Condition* condition = Condition::createCondition(
            CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, groupCooldown, 0,
            false, group);
        player->addCondition(condition);
      }
  ...
  if (selfTarget) {
    ...
    if (hasParam) {
      ...
      if (!target || target->getHealth() <= 0) {
        if (!casterTargetOrDirection) {
          if (cooldown > 0) {
            Condition* condition = Condition::createCondition(
                CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, cooldown, 0,
                false, spellId);
            player->addCondition(condition);
          }
    ...
    if (getHasPlayerNameParam()) {
      ...
      if (ret != RETURNVALUE_NOERROR) {
        ...
        if (groupCooldown > 0) {
          Condition* condition = Condition::createCondition(
              CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, groupCooldown,
              0, false, group);
          player->addCondition(condition);
        }
...
bool InstantSpell::internalCastSpell(Creature* creature,
...
bool InstantSpell::executeCastSpell(Creature* creature, const LuaVariant& var) {
  // onCastSpell(creature, var)
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - InstantSpell::executeCastSpell] Call stack overflow"
              << std::endl;
    return false;
  }
...
bool RuneSpell::configureEvent(const pugi::xml_node& node) {
  ...
  if (!(attr = node.attribute("id"))) {
    std::cout << "[Error - RuneSpell::configureSpell] Rune spell without id."
              << std::endl;
    return false;
  }
...
bool RuneSpell::executeUse(Player* player, Item* item, const Position&,
  ...
  postCastSpell(player);
  if (hasCharges && item &&
      g_config.getBoolean(ConfigManager::REMOVE_RUNE_CHARGES)) {
...
bool RuneSpell::executeCastSpell(Creature* creature, const LuaVariant& var,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - RuneSpell::executeCastSpell] Call stack overflow"
              << std::endl;
    return false;
  }
