...
bool Vocations::loadFromXml() {
  ...
  if (!result) {
    printXMLError("Error - Vocations::loadFromXml", "data/XML/vocations.xml",
                  result);
    return false;
  }
  ...
  for (auto vocationNode : doc.child("vocations").children()) {
    ...
    if (!(attr = vocationNode.attribute("id"))) {
      std::cout << "[Warning - Vocations::loadFromXml] Missing vocation id"
                << std::endl;
      continue;
    }
      ...
      if (strcasecmp(childNode.name(), "skill") == 0) {
        ...
        if (skillIdAttribute) {
          ...
          if (skill_id <= SKILL_LAST) {
            ...
            std::cout << "[Notice - Vocations::loadFromXml] No valid skill id: "
                      << skill_id << " for vocation: " << voc.id << std::endl;
          }
        } else {
          std::cout << "[Notice - Vocations::loadFromXml] Missing skill id for "
                       "vocation: "
                    << voc.id << std::endl;
        }
...
Vocation* Vocations::getVocation(uint16_t id) {
  ...
  if (it == vocationsMap.end()) {
    std::cout << "[Warning - Vocations::getVocation] Vocation " << id
              << " not found." << std::endl;
    return nullptr;
  }
  return &it->second;
...
uint64_t Vocation::getReqMana(uint32_t magLevel) {
  ...
  uint64_t reqMana = static_cast<uint64_t>(
      1600 *
