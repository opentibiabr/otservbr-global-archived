...
Imbuement* Imbuements::getImbuement(uint16_t id) {
  ...
  if (it == imbues.end()) {
    std::cout << "[Warning - Imbuements::getImbuement] Imbuement " << id
              << " not found" << std::endl;
    return nullptr;
  }
  return &it->second;
...
bool Imbuements::loadFromXml(bool /* reloading */) {
  ...
  for (auto baseNode : doc.child("imbuements").children()) {
    ...
    if (strcasecmp(baseNode.name(), "base") == 0) {
      ...
      if (!id) {
        std::cout
            << "[Warning - Imbuements::loadFromXml] Missing id for base entry"
            << std::endl;
        continue;
      }
      bases.emplace_back(
      ...
      pugi::xml_attribute id = baseNode.attribute("id");
      if (!id) {
        std::cout << "[Warning - Imbuements::loadFromXml] Missing id for "
                     "category entry"
                  << std::endl;
        continue;
      }
      categories.emplace_back(pugi::cast<uint16_t>(id.value()),
                              baseNode.attribute("name").as_string());
      ...
      if (!base) {
        std::cout
            << "[Warning - Imbuements::loadFromXml] Missing imbuement base id"
            << std::endl;
        continue;
      }
      ...
      if (groupBase == nullptr) {
        std::cout << "[Warning - Imbuements::loadFromXml] Group base '"
                  << baseid << "' not exist" << std::endl;
        continue;
      }
      ...
      if (!res.second) {
        std::cout << "[Warning - Imbuements::loadFromXml] Duplicate imbuement "
                     "of Base ID: '"
                  << baseid << "' ignored" << std::endl;
        continue;
      }
      ...
      if (!iconBase) {
        std::cout << "[Warning - Imbuements::loadFromXml] Missing iconid for "
                     "imbuement entry"
                  << std::endl;
        continue;
      }
      ...
      if (!categorybase) {
        std::cout
            << "[Warning - Imbuements::loadFromXml] Missing imbuement category"
            << std::endl;
        continue;
      }
      ...
      if (category_p == nullptr) {
        std::cout << "[Warning - Imbuements::loadFromXml] Category not exist"
                  << std::endl;
        continue;
      }
      ...
      if (!nameBase) {
        std::cout
            << "[Warning - Imbuements::loadFromXml] Missing imbuement name"
            << std::endl;
        continue;
      }
      ...
      for (auto childNode : baseNode.children()) {
        if (!(attr = childNode.attribute("key"))) {
          std::cout << "[Warning - Imbuements::loadFromXml] Missing key "
                       "attribute in imbuement id "
                    << runningid << std::endl;
          continue;
        }
        ...
        if (strcasecmp(type.c_str(), "item") == 0) {
          if (!(attr = childNode.attribute("value"))) {
            std::cout << "[Warning - Imbuements::loadFromXml] Missing item ID "
                         "for imbuement name '"
                      << imb.name << "'" << std::endl;
            continue;
          }
          ...
          if (!(attr = childNode.attribute("type"))) {
            std::cout << "[Warning - Imbuements::loadFromXml] Missing effect "
                         "type for imbuement name "
                      << imb.name << std::endl;
            continue;
          }
          ...
          if (strcasecmp(effecttype.c_str(), "skill") == 0) {
            ...
            if (tmpStrValue == "sword") {
              ...
              std::cout
                  << "[Warning - Imbuements::loadFromXml] Unknow skill name '"
                  << tmpStrValue << "' in imbuement name " << imb.name
                  << std::endl;
              continue;
            }

            if (!(attr = childNode.attribute("bonus"))) {
              std::cout << "[Warning - Imbuements::loadFromXml] Missing skill "
                           "bonus for imbuement name "
                        << imb.name << std::endl;
              continue;
            }
            ...
            if (!(attr = childNode.attribute("combat"))) {
              std::cout << "[Warning - Imbuements::loadFromXml] Missing combat "
                           "for imbuement name "
                        << imb.name << std::endl;
              continue;
            }
            ...
            if (combatType == COMBAT_NONE) {
              std::cout << "[Warning - Imbuements::loadFromXml] Unknown combat "
                           "type for element"
            ...
            if (!(attr = childNode.attribute("value"))) {
              std::cout << "[Warning - Imbuements::loadFromXml] Missing damage "
                           "reduction percentage for imbuement name "
                        << imb.name << std::endl;
              continue;
            }
            ...
            if (!(attr = childNode.attribute("value"))) {
              std::cout << "[Warning - Imbuements::loadFromXml] Missing speed "
                           "value for imbuement name "
                        << imb.name << std::endl;
              continue;
            }
            ...
            if (!(attr = childNode.attribute("value"))) {
              std::cout << "[Warning - Imbuements::loadFromXml] Missing cap "
                           "value for imbuement name "
                        << imb.name << std::endl;
              continue;
            }
...
BaseImbue* Imbuements::getBaseByID(uint16_t id) {
  auto it = std::find_if(
      ...
      [id](const BaseImbue& group_imb) { return group_imb.id == id; });
...
Category* Imbuements::getCategoryByID(uint16_t id) {
  auto it =
      std::find_if(categories.begin(), categories.end(),
                   [id](const Category& cat_imb) { return cat_imb.id == id; });
