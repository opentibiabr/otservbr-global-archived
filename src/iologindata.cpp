...
bool IOLoginData::savePlayer(Player* player)
{
  ...
  DBResult_ptr result = db.storeQuery(query.str());
  if (!result) {
    spdlog::warn("[IOLoginData::savePlayer] - Error for select result query from player: {}", player->getName());
    return false;
  }
  ...
  if (!db.executeQuery(query.str())) {
    spdlog::warn("[IOLoginData::savePlayer] - Error saving bestiary data from player: {}", player->getName());
    return false;
  }
  ...
  if (!db.executeQuery(query.str())) {
    spdlog::warn("[IOLoginData::savePlayer] - Error delete query 'player_items' from player: {}", player->getName());
    return false;
  }
  ...
  if (!saveItems(player, itemList, itemsQuery, propWriteStream)) {
    spdlog::warn("[IOLoginData::savePlayer] - Failed for save items from player: {}", player->getName());
    return false;
  }
  ...
  if (!db.executeQuery(query.str())) {
    spdlog::warn("[IOLoginData::savePlayer] - Failed to delete table 'prey_slosts' from player: {}", player->getName());
    return false;
  }
  ...
  if (!preyDataQuery.execute()) {
    spdlog::warn("[IOLoginData::savePlayer] - Failed for save prey from playerr: {}", player->getName());
    return false;
  }
  ...
  do {
    ...
    if (item) {
      if (!item->unserializeAttr(propStream)) {
        spdlog::warn("[IOLoginData::loadItems] - Failed to serialize");
      }
