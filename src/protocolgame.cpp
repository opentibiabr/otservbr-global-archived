...
void ProtocolGame::AddItem(NetworkMessage& msg, uint16_t id, uint8_t count) {
  ...
  if (it.stackable) {
    ...
    msg.addByte(fluidMap[count & 7]);
             ...
             player->getOperatingSystem() <= CLIENTOS_NEW_MAC) {
    msg.addByte(0x00);
  }
...
void ProtocolGame::login(const std::string& name, uint32_t accountId,
  ...
  if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
    ...
    if (!g_game.placeCreature(player, player->getLoginPosition())) {
      if (!g_game.placeCreature(player, player->getTemplePosition(), false,
                                true)) {
        disconnectClient(
    ...
    if (eventConnect != 0 ||
        !g_config.getBoolean(ConfigManager::REPLACE_KICK_ON_LOGIN)) {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg) {
  ...
  if (operatingSystem <= CLIENTOS_NEW_MAC) enableCompact();
  ...
  if (!Protocol::RSA_decrypt(msg)) {
    std::cout << "[ProtocolGame::onRecvFirstMessage] RSA Decrypt Failed"
              << std::endl;
    disconnect();
    return;
  }
  ...
  uint32_t accountId = IOLoginData::gameworldAuthentication(
      accountName, password, characterName);
  if (accountId == 0) {
...
void ProtocolGame::parsePacket(NetworkMessage& msg) {
  ...
  if (!player || player->isRemoved() || player->getHealth() <= 0) {
    if (recvbyte == 0x0F) {
      // we need to make the player pointer != null in this part, game.cpp
      // release is the first step login(player->getName(),
      // player->getAccount(), player->operatingSystem);
      disconnect();
      return;
    }
  ...
  if (recvbyte != 0xD3) {
    g_dispatcher.addTask(createTask(std::bind(
        &Modules::executeOnRecvbyte, g_modules, player, msg, recvbyte)));
  }
  ...
  switch (recvbyte) {
    ...
    case 0xD3:
      g_dispatcher.addTask(
          createTask(std::bind(&ProtocolGame::parseSetOutfit, this, msg)));
      break;
    case 0xD4:
    ...
    case 0xD5:
      ...
      break;
    case 0xD6:
      parseClearingImbuement(msg);
      break;
    case 0xD7:
      parseCloseImbuingWindow(msg);
      break;
    case 0xDC:
      parseAddVip(msg);
      break;
    ...
    case 0xFC:
      ...
      //		case 0xFE: parseStoreRequestTransactionHistory(msg);
      //break;

      // case 0xDF, 0xE0, 0xE1, 0xFB, 0xFC, 0xFD, 0xFE Premium Shop.

    default:
      // std::cout << "Player: " << player->getName() << " sent an unknown
      // packet header: 0x" << std::hex << static_cast<uint16_t>(recvbyte) <<
      // std::dec << "!" << std::endl;
      break;
  }
...
void ProtocolGame::parseSetOutfit(NetworkMessage& msg) {
  ...
  if (msg.getBufferPosition() == startBufferPosition) {
    ...
    if (outfitType == 0) {
      ...
      // outfit window dialog if try outfit is set to 2 it expects uint32_t value
      // after mounted and disable mounts from outfit window dialog
...
void ProtocolGame::parseUseItem(NetworkMessage& msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItem,
                   player->getID(), pos, stackpos, index, spriteId);
}
...
void ProtocolGame::parseUseItemEx(NetworkMessage& msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItemEx,
                   player->getID(), fromPos, fromStackPos, fromSpriteId, toPos,
...
void ProtocolGame::parseUseWithCreature(NetworkMessage& msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseWithCreature,
                   player->getID(), fromPos, fromStackPos, creatureId,
                   spriteId);
}
...
void ProtocolGame::parseThrow(NetworkMessage& msg) {
  ...
  if (toPos != fromPos) {
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerMoveThing,
                     player->getID(), fromPos, spriteId, fromStackpos, toPos,
                     count);
  }
}
...
void ProtocolGame::parseQuickLoot(NetworkMessage& msg) {
  ...
  uint16_t spriteId = msg.get<uint16_t>();
  uint8_t stackpos = msg.getByte();
  addGameTask(&Game::playerQuickLoot, player->getID(), pos, spriteId, stackpos,
              nullptr);
}
...
void ProtocolGame::parsePlayerPurchase(NetworkMessage& msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerPurchaseItem,
...
void ProtocolGame::parseRotateItem(NetworkMessage& msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerRotateItem,
                   player->getID(), pos, stackpos, spriteId);
}
...
void ProtocolGame::parseRuleViolationReport(NetworkMessage& msg) {
  ...
  if (reportType == REPORT_TYPE_NAME) {
    ...
    translation = msg.getString();
    msg.get<uint32_t>();  // statement id, used to get whatever player have
                          // said, we don't log that.
  }
...
void ProtocolGame::parseStoreRequestOffers(NetworkMessage& message) {
  ...
  if (index >= 0) {
    ...
    std::cout << "[Warning - ProtocolGame::parseStoreRequestOffers] requested "
                 "category: \""
              << categoryName << "\" doesn't exists" << std::endl;
  }
}
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage& msg) {
  ...
  if (amount > 0 && price > 0) {
    addGameTask(&Game::playerCreateMarketOffer, player->getID(), type, spriteId,
...
void ProtocolGame::sendBlessStatus() {
  ...
  // CLIENTOS_NEW_WINDOWS) ? 8 : 6; (compartability for the client 10) Ignore ToF
  // (bless 1)
  ...
  msg.add<uint16_t>(
      (blessCount >= 5)
          ? (flag | 1)
          : flag);  // Show up the glowing effect in items if have all blesses
...
void ProtocolGame::sendPremiumTrigger() {
  if (!g_config.getBoolean(ConfigManager::FREE_PREMIUM)) {
    ...
    for (uint16_t i = 0; i <= 15; i++) {
      ...
      // false, PREMIUM_TRIGGER_MARKET = false, PREMIUM_TRIGGER_VIP_LIST = false,
      // PREMIUM_TRIGGER_DEPOT_SPACE = false, PREMIUM_TRIGGER_INVITE_PRIVCHAT =
      // false
      msg.addByte(0x01);
    }
    writeToOutputBuffer(msg);
...
void ProtocolGame::sendPreyRerollPrice(uint32_t price /*= 0*/,
                                       uint8_t wildcard /*= 0*/,
                                       uint8_t directly /*= 0*/) {
  NetworkMessage msg;
...
void ProtocolGame::sendSaleItemList(
    const std::vector<ShopInfo>& shop,
  ...
  do {
    for (auto item : stashToSend) {
      const ItemType& itemType =
          Item::items[Item::items.getItemIdByClientId(item.first).id];
      if (itemType.wareId == 0) {
        continue;
      }
...
void ProtocolGame::sendMarketDetail(uint16_t itemId) {
  ...
  if (statistics) {
    ...
    msg.add<uint32_t>(std::min<uint64_t>(std::numeric_limits<uint32_t>::max(),
...
void ProtocolGame::sendAddCreature(const Creature* creature,
  ...
  msg.add<uint16_t>(static_cast<uint16_t>(
      g_config.getNumber(ConfigManager::STORE_COIN_PACKET)));

  msg.addByte(shouldAddExivaRestrictions ? 0x01
                                         : 0x00);  // exiva button enabled
...
void ProtocolGame::sendImbuementWindow(Item* item) {
  ...
  if (!itemHasImbue && imbuements.empty()) {
    player->sendTextMessage(
        ...
        "You did not collect enough knowledge from the ancient Shapers. Visit "
        "the Shaper temple in Thais for help.");
    return;
  }
  // Seting imbuing item
  ...
  for (uint8_t i = 0; i < slot; i++) {
    ...
    if (info >> 8) {
      ...
      msg.add<uint32_t>(
          g_imbuements
              ->getBaseByID(
                  g_imbuements->getImbuement((info & 0xFF))->getBaseID())
              ->removecust);
    } else {
      msg.addByte(0x00);
    }
...
void ProtocolGame::reloadCreature(const Creature* creature) {
  ...
  std::unordered_set<uint32_t>::iterator it = std::find(
...
void ProtocolGame::AddPlayerStowedItems(NetworkMessage& msg) {
  ...
  msg.add<uint16_t>(g_config.getNumber(ConfigManager::STASH_ITEMS) -
                    IOStash::getStashSize(list));
}
...
void ProtocolGame::sendLockerItems(std::map<uint16_t, uint16_t> itemMap,
                                   uint16_t count) {
  NetworkMessage msg;
  msg.addByte(0x94);
