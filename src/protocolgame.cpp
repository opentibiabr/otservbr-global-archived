...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	switch (recvbyte) {
		...
		case 0x77: onPressHotkeyEquip(msg); break;
...
void ProtocolGame::onPressHotkeyEquip(NetworkMessage& msg)
{
	if (!player) {
		return;
	}
	...
	item = g_game.findItemOfType(player, itemType.id);

	if (item == nullptr) {
		item = g_game.findItemOfType(player, itemType.transformEquipTo);
		if (item == nullptr) {
			item = g_game.findItemOfType(player, itemType.transformDeEquipTo);
		}
		if (item == nullptr) {
	...
	if (hasBitSet(SLOTP_TWO_HAND, slotP)) {
	 Thing* leftthing = player->getThing(CONST_SLOT_LEFT);
	  if (leftthing) {
		Item* slotLeft_item = leftthing->getItem();
		  if (slotLeft_item) {
		   ...
		   ret = g_game.internalMoveItem(slotLeft_item->getParent(), player, 0, slotLeft_item, slotLeft_item->getItemCount(), nullptr);	
		  }
	  }
	  Thing* rightthing = player->getThing(CONST_SLOT_RIGHT);
		if (rightthing) {
		  Item* slotRight_Item = rightthing->getItem();
			if (slotRight_Item) {
			  ret = g_game.internalMoveItem(slotRight_Item->getParent(), player, 0, slotRight_Item, slotRight_Item->getItemCount(), nullptr);
			} else {
			  return;
			}
		}
	   if (!removed) {
		ret = g_game.internalMoveItem(item->getParent(), player, CONST_SLOT_LEFT, item, item->getItemCount(), nullptr);
	   }
	} else if (hasBitSet(SLOTP_RING, slotP)) {
	  Thing* ringthing = player->getThing(CONST_SLOT_RING);
		if (ringthing) {
		  Item* slotRing_Item = ringthing->getItem();
			if (slotRing_Item) {
			  ret = g_game.internalMoveItem(slotRing_Item->getParent(), player, 0, slotRing_Item, slotRing_Item->getItemCount(), nullptr);
		...
		if (!removed) {
		 ret = g_game.internalMoveItem(item->getParent(), player, CONST_SLOT_RING, item, item->getItemCount(), nullptr);
		}
	  ...
	  Thing* leftthing = player->getThing(CONST_SLOT_LEFT);
		...
		if (!removed) {
		  ret = g_game.internalMoveItem(item->getParent(), player, slotP, item, item->getItemCount(), nullptr);
		}
	} else if (slotP) {
	  if (newitemType.stackable) {
				...
				if (ammoItem->getID() == item->getID()) {
				  ...
				  ammoItem->getItemCount() == player->getItemTypeCount(ammoItem->getID())) {
					...
					if (ret != RETURNVALUE_NOERROR) {
					 ret = g_game.internalMoveItem(ammoItem->getParent(), player, 0, ammoItem, ammoItem->getItemCount(), nullptr);
					}
					if (ret != RETURNVALUE_NOERROR) {
		...
		item->getDecaying() != DECAYING_TRUE) {
		  ...
		  uint16_t count = 0;
			while (player->getItemTypeCount(item->getID())) {
			  ...
			  Item* _item = g_game.findItemOfType(mainBP, itemId);
			  ...
			  if (_item->getItemCount() > 100 - count) {
				g_game.internalRemoveItem(_item, 100 - count);
				count = 100;
			  } else {
				count = count + _item->getItemCount();
				g_game.internalRemoveItem(_item, _item->getItemCount());
			  }
			}
		  Item* newSlotitem = Item::CreateItem(itemId, count);
		  g_game.internalAddItem(player, newSlotitem, slotP, FLAG_NOLIMIT);
		  return;
		} else {
		  ret = g_game.internalMoveItem(item->getParent(), player, slotP, item, item->getItemCount(), nullptr);
		}
		...
		if (slotthing) {
		  Item* slotItem = slotthing->getItem();
			if (slotItem) {
			  ret = g_game.internalMoveItem(slotItem->getParent(), player, 0, slotItem, slotItem->getItemCount(), nullptr);
