...
bool Scripts::loadScripts(std::string folderName, bool isLib, bool reload) {
  ...
  if (!fs::exists(dir) || !fs::is_directory(dir)) {
    std::cout << "[Warning - Scripts::loadScripts] Can not load folder '"
              << folderName << "'." << std::endl;
    return false;
  }
    ...
    if (fs::is_regular_file(*it) && it->path().extension() == ".lua") {
      ...
      if (found != std::string::npos) {
        if (g_config.getBoolean(ConfigManager::SCRIPTS_CONSOLE_LOGS)) {
          std::cout << "> " << it->path().filename().string() << " [disabled]"
                    << std::endl;
        }
        continue;
      }
  ...
  for (auto it = v.begin(); it != v.end(); ++it) {
    ...
    if (g_config.getBoolean(ConfigManager::SCRIPTS_CONSOLE_LOGS)) {
      if (!reload) {
        std::cout << "> " << it->filename().string() << " [loaded]"
                  << std::endl;
      } else {
        std::cout << "> " << it->filename().string() << " [reloaded]"
                  << std::endl;
      }
    }
  }
