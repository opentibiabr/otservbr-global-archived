...
static constexpr int32_t INDEX_WHEREVER = -1;

enum cylinderflags_t {
  FLAG_NOLIMIT = 1 << 0,  // Bypass limits like capacity/container limits,
  ...
  FLAG_CHILDISOWNER =
      1 << 3,  // Used by containers to query capacity of the carrier (player)
  FLAG_PATHFINDING =
      1 << 4,  // An additional check is done for floor changing/teleport items
  ...
  FLAG_IGNOREAUTOSTACK =
      1 << 7,  // queryDestination will not try to stack items together
};
...
class VirtualCylinder final : public Cylinder {
 public:
  ...
  virtual ReturnValue queryRemove(const Thing&, uint32_t,
                                  uint32_t) const override {
    return RETURNVALUE_NOTPOSSIBLE;
  }
