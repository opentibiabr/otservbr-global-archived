...
bool Actions::registerEvent(Event_ptr event, const pugi::xml_node& node) {
  ...
  if ((attr = node.attribute("itemid"))) {
    ...
    if (!result.second) {
      std::cout << "[Warning - Actions::registerEvent] Duplicate \
			registered item with id: "
                << iterId << " in fromid: " << fromId << ", toid: " << toId
                << std::endl;
    }
    ...
    while (++iterId <= toId) {
      ...
      if (!result.second) {
        std::cout << "[Warning - Actions::registerEvent] Duplicate registered "
                     "item with id: "
                  << iterId << " in fromid: " << fromId << ", toid: " << toId
                  << std::endl;
        continue;
    ...
    if (!toUidAttribute) {
      std::cout
          << "[Warning - Actions::registerEvent] Missing touid in fromuid: "
          << attr.as_string() << std::endl;
      return false;
    }
    ...
    if (!result.second) {
      std::cout << "[Warning - Actions::registerEvent] Duplicate registered "
                   "item with unique id: "
                << iterUid << " in fromuid: " << fromUid << ", touid: " << toUid
                << std::endl;
    }
    ...
    if (!toAidAttribute) {
      std::cout
          << "[Warning - Actions::registerEvent] Missing toaid in fromaid: "
          << attr.as_string() << std::endl;
      return false;
    }
    ...
    if (!result.second) {
      std::cout << "[Warning - Actions::registerEvent] Duplicate registered "
                   "item with action id: "
                << iterAid << " in fromaid: " << fromAid << ", toaid: " << toAid
                << std::endl;
    }
    ...
    while (++iterAid <= toAid) {
      ...
      if (!result.second) {
        std::cout << "[Warning - Actions::registerEvent] Duplicate registered "
                     "item with action id: "
                  << iterAid << " in fromaid: " << fromAid
                  << ", toaid: " << toAid << std::endl;
        continue;
      }
      success = true;
    }
  ...
  if (action->getItemIdRange().size() > 0) {
    if (action->getItemIdRange().size() == 1) {
      auto result = useItemMap.emplace(action->getItemIdRange().at(0),
                                       std::move(*action));
      if (!result.second) {
        std::cout << "[Warning - Actions::registerLuaEvent] Duplicate "
                     "registered item with id: "
                  << action->getItemIdRange().at(0) << std::endl;
      }
      return result.second;
    } else {
      ...
      for (auto i = v.begin(); i != v.end(); i++) {
        ...
        if (!result.second) {
          std::cout << "[Warning - Actions::registerLuaEvent] Duplicate "
                       "registered item with id: "
                    << *i << " in range from id: " << v.at(0)
                    << ", to id: " << v.at(v.size() - 1) << std::endl;
          continue;
        }
      }
      return true;
    ...
    if (action->getUniqueIdRange().size() == 1) {
      auto result = uniqueItemMap.emplace(action->getUniqueIdRange().at(0),
                                          std::move(*action));
      if (!result.second) {
        std::cout << "[Warning - Actions::registerLuaEvent] Duplicate "
                     "registered item with uid: "
                  << action->getUniqueIdRange().at(0) << std::endl;
      }
      return result.second;
    } else {
    ...
    if (action->getActionIdRange().size() == 1) {
      auto result = actionItemMap.emplace(action->getActionIdRange().at(0),
                                          std::move(*action));
      if (!result.second) {
        std::cout << "[Warning - Actions::registerLuaEvent] Duplicate "
                     "registered item with aid: "
                  << action->getActionIdRange().at(0) << std::endl;
      }
      return result.second;
    } else {
    ...
    std::cout << "[Warning - Actions::registerLuaEvent] There is no id / aid / "
                 "uid set for this event"
              << std::endl;
    return false;
  }
}
...
bool Actions::useItem(Player* player, const Position& pos, uint8_t index,
  ...
  if (it.isRune() || it.type == ITEM_TYPE_POTION) {
    ...
    player->setNextPotionAction(
        OTSYS_TIME() +
        g_config.getNumber(ConfigManager::ACTIONS_DELAY_INTERVAL));
  } else {
...
bool enterStash(Player* player, Item*, const Position&, Thing*, const Position&,
                bool) {
  if (!player) {
    return false;
  }
...
bool Action::executeUse(Player* player, Item* item,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    std::cout << "[Error - Action::executeUse] Call stack overflow"
              << std::endl;
    return false;
  }
