...
void printXMLError(const std::string& where, const std::string& fileName,
  ...
  std::cout << '[' << where << "] Failed to load " << fileName << ": "
...
using CombatTypeNames =
    std::unordered_map<CombatType_t, std::string, std::hash<int32_t>>;
...
const char* getReturnMessage(ReturnValue value) {
  switch (value) {
    case RETURNVALUE_REWARDCHESTISEMPTY:
      return "The chest is currently empty. You did not take part in any "
             "battles in the last seven days or already claimed your reward.";
