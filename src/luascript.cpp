...
void LuaScriptInterface::reportError(const char* function,
...
// Metatables
void LuaScriptInterface::setMetatable(lua_State* L, int32_t index,
                                      const std::string& name) {
  luaL_getmetatable(L, name.c_str());
  lua_setmetatable(L, index - 1);
}

void LuaScriptInterface::setWeakMetatable(lua_State* L, int32_t index,
                                          const std::string& name) {
...
void LuaScriptInterface::setItemMetatable(lua_State* L, int32_t index,
                                          const Item* item) {
...
std::string LuaScriptInterface::getFieldString(lua_State* L, int32_t arg,
...
void LuaScriptInterface::pushCombatDamage(lua_State* L,
...
void LuaScriptInterface::pushInstantSpell(lua_State* L,
...
void LuaScriptInterface::pushLoot(lua_State* L,
...
#define registerEnum(value)                                                 \
  {                                                                         \
    ...
    registerGlobalVariable(enumName.substr(enumName.find_last_of(':') + 1), \
                           value);                                          \
  }
#define registerEnumIn(tableName, value)                                      \
  {                                                                           \
    std::string enumName = #value;                                            \
    registerVariable(tableName,                                               \
                     enumName.substr(enumName.find_last_of(':') + 1), value); \
  }
...
void LuaScriptInterface::registerFunctions()
{
	...
	// _G
	registerGlobalVariable("INDEX_WHEREVER", INDEX_WHEREVER);
...
void LuaScriptInterface::registerClass(
    const std::string& className, const std::string& baseClass,
...
void LuaScriptInterface::registerGlobalMethod(const std::string& functionName,
                                              lua_CFunction func) {
...
void LuaScriptInterface::registerVariable(const std::string& tableName,
                                          const std::string& name,
                                          lua_Number value) {
...
void LuaScriptInterface::registerGlobalVariable(const std::string& name,
                                                lua_Number value) {
  // _G[name] = value
...
void LuaScriptInterface::registerGlobalBoolean(const std::string& name,
                                               bool value) {
  // _G[name] = value
...
int LuaScriptInterface::luaDoPlayerAddItem(lua_State* L) {
  ...
  // <optional: default: 1> canDropOnMap) doPlayerAddItem(cid, itemid, <optional:
  // default: 1> count, <optional: default: 1> canDropOnMap, <optional: default:
  // 1>subtype)
...
bool LuaScriptInterface::getArea(lua_State* L, std::list<uint32_t>& list,
                                 uint32_t& rows) {
  lua_pushnil(L);
...
int LuaScriptInterface::luaIsValidUID(lua_State* L) {
  // isValidUID(uid)
  pushBoolean(
      L, getScriptEnv()->getThingByUID(getNumber<uint32_t>(L, -1)) != nullptr);
  return 1;
}
...
int LuaScriptInterface::luaConfigManagerGetString(lua_State* L) {
  pushString(
      L, g_config.getString(getNumber<ConfigManager::string_config_t>(L, -1)));
  return 1;
}

int LuaScriptInterface::luaConfigManagerGetNumber(lua_State* L) {
  lua_pushnumber(
      L, g_config.getNumber(getNumber<ConfigManager::integer_config_t>(L, -1)));
  return 1;
}

int LuaScriptInterface::luaConfigManagerGetBoolean(lua_State* L) {
  pushBoolean(L, g_config.getBoolean(
                     getNumber<ConfigManager::boolean_config_t>(L, -1)));
  return 1;
}
...
int LuaScriptInterface::luaGameLoadMap(lua_State* L) {
  ...
  g_dispatcher.addTask(createTask([path]() {
    try {
      ...
      std::cout
          << "[Error - LuaScriptInterface::luaGameLoadMap] Failed to load map: "
          << e.what() << std::endl;
    }
  }));
  return 0;
...
int LuaScriptInterface::luaGameCreateItem(lua_State* L) {
  ...
  for (int32_t i = 1; i <= itemCount; ++i) {
    ...
    if (position.x != 0) {
      ...
      ReturnValue ret =
          g_game.internalAddItem(tile, item, INDEX_WHEREVER, FLAG_NOLIMIT);
      if (ret != RETURNVALUE_NOERROR) {
        delete item;
        if (!hasTable) {
          lua_pushnil(L);
...
int LuaScriptInterface::luaGameCreateContainer(lua_State* L) {
  ...
  if (lua_gettop(L) >= 3) {
    ...
    g_game.internalAddItem(tile, container, INDEX_WHEREVER, FLAG_NOLIMIT);
  } else {
...
int LuaScriptInterface::luaGameCreateMonsterType(lua_State* L) {
  ...
  if (getScriptEnv()->getScriptInterface() !=
    ...
    reportErrorFunc(
        "MonsterTypes can only be registered in the Scripts interface.");
    lua_pushnil(L);
    return 1;
  }
...
int LuaScriptInterface::luaTileAddItem(lua_State* L) {
  ...
  ReturnValue ret = g_game.internalAddItem(tile, item, INDEX_WHEREVER, flags);
  if (ret == RETURNVALUE_NOERROR) {
    pushUserdata<Item>(L, item);
    setItemMetatable(L, -1, item);
  } else {
...
int LuaScriptInterface::luaItemRemove(lua_State* L) {
  ...
  if (item) {
    int32_t count = getNumber<int32_t>(L, 2, -1);
    pushBoolean(L,
                g_game.internalRemoveItem(item, count) == RETURNVALUE_NOERROR);
  } else {
    lua_pushnil(L);
  }
  return 1;
...
int LuaScriptInterface::luaItemMoveTo(lua_State* L) {
  ...
  if (item->getParent() == VirtualCylinder::virtualCylinder) {
    pushBoolean(L, g_game.internalAddItem(toCylinder, item, INDEX_WHEREVER,
                                          flags) == RETURNVALUE_NOERROR);
  } else {
    Item* moveItem = nullptr;
    ReturnValue ret =
        g_game.internalMoveItem(item->getParent(), toCylinder, INDEX_WHEREVER,
                                item, item->getItemCount(), &moveItem, flags);
    if (moveItem) {
      *itemPtr = moveItem;
    }
...
int LuaScriptInterface::luaContainerAddItem(lua_State* L) {
  // container:addItem(itemId[, count/subType = 1[, index = INDEX_WHEREVER[,
  // flags = 0]]])
  Container* container = getUserdata<Container>(L, 1);
  if (!container) {
    lua_pushnil(L);
  ...
  int32_t index = getNumber<int32_t>(L, 4, INDEX_WHEREVER);
...
int LuaScriptInterface::luaContainerAddItemEx(lua_State* L) {
  // container:addItemEx(item[, index = INDEX_WHEREVER[, flags = 0]])
  Item* item = getUserdata<Item>(L, 2);
  if (!item) {
    lua_pushnil(L);
    return 1;
  ...
  int32_t index = getNumber<int32_t>(L, 3, INDEX_WHEREVER);
  uint32_t flags = getNumber<uint32_t>(L, 4, 0);
  ReturnValue ret = g_game.internalAddItem(container, item, index, flags);
  if (ret == RETURNVALUE_NOERROR) {
    ScriptEnvironment::removeTempItem(item);
...
int LuaScriptInterface::luaCreatureReload(lua_State* L) {
  ...
  SpectatorHashSet spectators;
  g_game.map.getSpectators(
      spectators, position, false,
      true);  // 3 parâmetro é multifloor, ver se há necessidade de usar.
...
int LuaScriptInterface::luaPlayerAddItemEx(lua_State* L) {
  // player:addItemEx(item[, canDropOnMap = false[, index = INDEX_WHEREVER[,
...
int LuaScriptInterface::luaPlayerSendContainer(lua_State* L) {
  ...
  player->sendContainer(container->getClientID(), container,
                        container->hasParent(), container->getFirstIndex());
  pushBoolean(L, true);
  return 1;
}
    ...
    if (lua_gettop(L) == 1) {
      ...
      if (immunity == "physical") {
        ...
        std::cout << "[Warning - Monsters::loadMonster] Unknown immunity name "
                  << immunity << " for monster: " << monsterType->name
                  << std::endl;
        lua_pushnil(L);
      }
    }
  ...
  if (monsterType) {
    ...
    if (spell) {
      ...
      if (g_monsters.deserializeSpell(spell, sb, monsterType->name)) {
        ...
        std::cout << "[Warning - Monsters::loadMonster] Cant load spell. "
                  << spell->name << std::endl;
      }
    } else {
      lua_pushnil(L);
...
void LuaScriptInterface::parseLoot(lua_State* L,
                                   const std::vector<LootBlock>& lootList) {
  lua_createtable(L, lootList.size(), 0);
  ...
  if (monsterType) {
    if (lua_gettop(L) == 1) {
      ...
      if (race == "venom") {
        ...
        std::cout << "[Warning - Monsters::loadMonster] Unknown race type "
                  << race << "." << std::endl;
        lua_pushnil(L);
        return 1;
      }
    ...
    if (lua_gettop(L) == 1) {
      ...
      if (group) {
        ...
        if (group != SPELLGROUP_NONE) {
          ...
          std::cout << "[Warning - Spell::group] Unknown group: "
                    << getString(L, 2) << std::endl;
          pushBoolean(L, false);
          return 1;
        }
        pushBoolean(L, true);
      ...
      if (primaryGroup && secondaryGroup) {
        ...
        if (primaryGroup != SPELLGROUP_NONE) {
          ...
          std::cout << "[Warning - Spell::group] Unknown primaryGroup: "
                    << getString(L, 2) << std::endl;
          pushBoolean(L, false);
          return 1;
        }
        ...
        if (secondaryGroup != SPELLGROUP_NONE) {
          ...
          std::cout << "[Warning - Spell::group] Unknown secondaryGroup: "
                    << getString(L, 3) << std::endl;
          pushBoolean(L, false);
          return 1;
        }
        pushBoolean(L, true);
...
int LuaScriptInterface::luaCreatureEventType(lua_State* L) {
  ...
  if (creature) {
    ...
    if (tmpStr == "login") {
      ...
      std::cout << "[Error - CreatureEvent::configureLuaEvent] Invalid type "
                   "for creature event: "
                << typeName << std::endl;
      pushBoolean(L, false);
    }
    ...
    if (moveevent->getEventType() == MOVE_EVENT_EQUIP ||
      ...
      if (!moveevent->getSlotName().empty()) {
        ...
        if (tmpStr == "head") {
          ...
          std::cout
              << "[Warning - MoveEvent::configureMoveEvent] Unknown slot type: "
              << moveevent->getSlotName() << std::endl;
        }
      }
    }
    pushBoolean(L, true);
...
int LuaScriptInterface::luaGlobalEventTime(lua_State* L) {
  ...
  if (globalevent) {
    ...
    if (hour < 0 || hour > 23) {
      std::cout << "[Error - GlobalEvent::configureEvent] Invalid hour \""
                << timer
    ...
    if (params.size() > 1) {
      min = params[1];
      if (min < 0 || min > 59) {
        std::cout << "[Error - GlobalEvent::configureEvent] Invalid minute \""
                  << timer
                  << "\" for globalevent with name: " << globalevent->getName()
                  << std::endl;
        pushBoolean(L, false);
      ...
      if (params.size() > 2) {
        sec = params[2];
        if (sec < 0 || sec > 59) {
          std::cout << "[Error - GlobalEvent::configureEvent] Invalid second \""
                    << timer << "\" for globalevent with name: "
...
int LuaScriptInterface::luaCreateWeapon(lua_State* L) {
  ...
  switch (type) {
    ...
    case WEAPON_CLUB: {
      WeaponMelee* weapon =
          new WeaponMelee(getScriptEnv()->getScriptInterface());
      if (weapon) {
  ...
  if (weapon) {
    if (!getNumber<CombatType_t>(L, 2)) {
      ...
      if (tmpStrValue == "earth") {
        ...
        std::cout << "[Warning - weapon:element] Type \"" << element
...
int LuaScriptInterface::luaWeaponAmmoType(lua_State* L) {
  ...
  if (weapon) {
    ...
    if (type == "arrow") {
      ...
      std::cout << "[Warning - weapon:ammoType] Type \"" << type
                << "\" does not exist." << std::endl;
      lua_pushnil(L);
      return 1;
    }
  ...
  if (weapon) {
    ...
    if (!getNumber<CombatType_t>(L, 3)) {
      ...
      if (tmpStrValue == "earth") {
        ...
        std::cout << "[Warning - weapon:extraElement] Type \"" << element
                  << "\" does not exist." << std::endl;
      }
    } else {
...
void LuaEnvironment::executeTimerEvent(uint32_t eventIndex) {
  ...
  if (reserveScriptEnv()) {
    ...
    std::cout
        << "[Error - LuaScriptInterface::executeTimerEvent] Call stack overflow"
        << std::endl;
  }
